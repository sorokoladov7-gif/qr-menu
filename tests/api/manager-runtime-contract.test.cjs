...
test('manager hall browser runtime stays on the canonical table RPC family', () => {
  ...
  for (const name of ['manager_create_table','manager_update_table','manager_move_table','manager_delete_table','manager_regenerate_table_qr','manager_set_table_status','manager_seat_table','manager_set_table_reservation_guest','manager_close_table_session','manager_save_hall_plan','manager_delete_hall_plan']) {
...
test('manager recipes keep reads separate from canonical mutation bridge', () => {
  ...
  assertNoLegacy(source, ['manager_recipe_auto_sync','manager_global_ingredient_update','manager_global_ingredient_delete'], 'manager-recipes.js');
