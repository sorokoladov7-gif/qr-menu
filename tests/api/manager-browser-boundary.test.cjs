...
test('manager order status mutations use canonical action API',()=>{const source=readAsset('manager-orders.js');assert.ok(source.includes("runner({type:'update_order'"));assert.ok(source.includes('venue_id:venueId'));assert.equal(source.includes("db.from('orders').update(u)"),false);});
