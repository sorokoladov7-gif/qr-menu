...
test('protected API endpoints reject unauthenticated requests', async (t) => {
  if (!base) return t.skip('TEST_API_BASE_URL or PLAYWRIGHT_BASE_URL is not configured');
...
test('admin payment confirmation uses the canonical plan and entitlement chain', () => {
  ...
  const source=fs.readdirSync(dir).filter(name=>name.endsWith('.sql')&&name>='20260910100000_harden_admin_payment_entitlement_chain.sql').map(name=>read(`supabase/migrations/${name}`)).join('\n');
...
test('manager entitlement RPCs serialize per-manager mutations',()=>{const source=read('supabase/migrations/20260910101000_lock_manager_entitlement_rpcs.sql');assert.equal((source.match(/pg_advisory_xact_lock\(hashtextextended\(p_manager_id::text, 0\)\)/g)||[]).length,2);assert.match(source,/limit 1\s+for update/);});

test('manager AI action gateway keeps feature and mutation boundaries in canonical modules',()=>{const source=read('lib/ai/manager/action.js');assert.match(source,/MAP\[f\].*includes\(type\)/);assert.match(source,/entitlement\(c,f\)/);assert.match(source,/menu\.run\(type,c,vid,p/);assert.match(source,/ingredients\.run\(type,c,vid,p/);assert.match(source,/staff\.run\(type,c,vid,p/);assert.match(source,/hall\.run\(type,c,vid,p/);});

test('manager support thread creation enforces supplied venue ownership',()=>{const rpcSource=read('supabase/migrations/20260910180000_harden_manager_support_thread_venue_scope.sql');const rlsSource=read('supabase/migrations/20260910181500_harden_manager_support_thread_rls_venue_scope.sql');assert.match(rpcSource,/p_venue_id is not null and not public\.is_manager_of\(p_venue_id\)/);assert.match(rpcSource,/raise exception 'VENUE_ACCESS_DENIED'/);assert.match(rlsSource,/manager_support_threads_manager_insert/);assert.match(rlsSource,/venue_id is null or public\.is_manager_of\(venue_id\)/);assert.match(rlsSource,/manager_support_threads_manager_update/);});
