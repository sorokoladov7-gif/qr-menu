...
(function () {
  ...
  async function api(payload) {
    ...
    if (!r.ok) throw new Error(d.error || 'Qrchick: ошибка выполнения');
    return d;
  }
  function avatar(sizeClass) {
    return '<img class="qc-avatar-img ' + (sizeClass || '') + '" src="/assets/img/qrchick-avatar.svg" alt="Qrchick">';
  }
  function render() {
    var box = $('qc-messages');
    if (!box) return;
    box.innerHTML = history.map(function (m) {
      ...
      return '<article class="qc-message ' + (user ? 'qc-user' : 'qc-ai') + '"><div class="qc-msg-wrap">' +
        (user ? '' : '<div class="qc-avatar">' + avatar() + '</div>') +
  ...
  async function run(message) {
    ...
    render(); activity('Qrchick выполняет запрос…'); status('Работает');
    try {
      var ctx = '';
      ...
      var answer = result.answer || result.summary || '';
      if (!answer) {
        if (result.findings && result.findings.length) answer = 'Я закончил проверку. Найдено проблем: ' + result.findings.length + '.';
        else if (result.actions && result.actions.length) answer = 'Задача разобрана. Я подготовил план действий ниже.';
        else if (result.proposed_changes && result.proposed_changes.length) answer = 'Я подготовил изменения кода. Проверьте их ниже и подтвердите применение.';
        else if (result.database_changes && result.database_changes.length) answer = 'Я подготовил изменения базы данных. Проверьте их ниже и подтвердите выполнение.';
        else answer = 'Готов. Запрос обработан.';
      }
      history.push({ role: 'assistant', content: answer });
      save(); render(); showResult(d); activity('Qrchick готов'); status('Готов');
    } catch (e) {
  ...
  function showResult(d) {
    ...
    if (pendingChanges.length) html += '<div class="qc-card"><b>Изменения кода · ' + pendingChanges.length + '</b><div class="qc-muted">Требуется подтверждение администратора.</div><button id="qc-apply-code" class="qc-apply" type="button">Применить</button></div>';
    if (pendingDbChanges.length) html += '<div class="qc-card"><b>Изменения БД · ' + pendingDbChanges.length + '</b><div class="qc-muted">Изменение данных выполняется только после подтверждения администратора.</div><button id="qc-apply-db" class="qc-apply" type="button">Применить</button></div>';
    box.innerHTML = html;
  ...
  async function applyCode() {
    if (!pendingChanges.length || !window.confirm('Применить подготовленные изменения кода?')) return;
  ...
  async function applyDb() {
    if (!pendingDbChanges.length || !window.confirm('Выполнить подготовленные изменения базы данных?')) return;
    try { var d = await api({ action: 'apply_db', database_changes: pendingDbChanges }); pendingDbChanges = []; $('qc-result').innerHTML = '<div class="qc-card"><b>Изменения базы данных применены</b><pre class="qc-code">' + esc(JSON.stringify(d.changes || [], null, 2)) + '</pre></div>'; } catch (e) { status('Ошибка БД: ' + e.message); }
  }
  ...
  function showWelcome() {
    if (!history.length) { history.push({ role: 'assistant', content: 'Я Qrchick — инженерный ИИ, созданный разработчиком платформы специально для QR Menu. Я могу анализировать и изменять код, работать с базой данных, проверять RLS/RPC, Vercel, импорт, файлы и данные. Опасные изменения выполняются только после твоего подтверждения.' }); save(); }
    render();
  }
