...
(function(){
  ...
  /* Final billing authority: canonical admin methods must survive older Vue.createApp wrappers. */
    ...
    Vue.createApp = function(options) {
      if (options && typeof options === 'object') {
        ...
        Object.keys(canonical).forEach(function(key) {
          options.methods[key] = canonical[key];
        });

        /* app.js and admin-managers.js still wrap createApp. Ignore later legacy assignments to these keys. */
        var protectedKeys = { changeManagerPlan: true, extendManagerSub: true, changePlan: true, extendSub: true };
        var targetMethods = options.methods;
        options.methods = new Proxy(targetMethods, {
          set: function(target, key, value) {
            if (protectedKeys[key]) return true;
            target[key] = value;
            return true;
          },
          ...
          deleteProperty: function(target, key) {
            if (protectedKeys[key]) return false;
