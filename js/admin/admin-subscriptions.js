...
(function(){
  ...
  function syncAIState(plan){
    if(!plan)return;
    var next=featureObject(plan), changed=false;
    Object.keys(next).forEach(function(k){
      if(!plan.ai_features || plan.ai_features[k]!==next[k]) changed=true;
    });
    if(!plan.ai_features || changed) plan.ai_features=next;
    plan.ai_enabled=Object.keys(next).some(function(k){return next[k];});
  }
  ...
  var subsMixin={
    ...
    computed:{
      ...
      subStats:function(){
        ...
        return {total:(this.subscriptions||[]).filter(function(s){return s&&s.manager_id;}).length,active:active,trial:trial,expired:expired,expiringSoon:expiring};
      },
      venueSubs:function(){
        var self=this;
      ...
      planStats:function(){
        ...
        return this.plans.map(function(p){
          ...
          return {id:p.id,name:p.name,price:Number(p.price)||0,count:ss.length,active:active,mrr:active*(Number(p.price)||0),is_public:p.is_public!==false,ai_enabled:p.ai_enabled===true,ai_count:countFeatures(p)};
        });
      }
    },
    methods:{
      ...
      changeManagerPlan:async function(m,plan){
        var managerId=m&&m.id,planId=String(plan||'').trim();
        if(!managerId){this.msg='Не найден управляющий';return;}
        if(!planId){this.msg='Выберите тариф';return;}
        var selected=(this.plans||[]).find(function(p){return p.id===planId;});
        if(!selected){this.msg='Выбранный тариф не найден';return;}
        this.busy=true;
        try{var r=await db.rpc('admin_set_manager_plan',{p_manager_id:managerId,p_plan_id:planId,p_days:null});if(r.error)throw r.error;await this.loadBaseData();this.msg='Тариф «'+selected.name+'» назначен управляющему';}
        catch(e){this.msg='Ошибка сохранения тарифа: '+(e.message||e);}
        finally{this.busy=false;}
      },
      extendManagerSub:async function(m,days){
        var managerId=m&&m.id;days=Number(days)||30;
        if(!managerId){this.msg='Не найден управляющий';return;}
        this.busy=true;
        try{var r=await db.rpc('admin_extend_manager_subscription',{p_manager_id:managerId,p_days:days});if(r.error)throw r.error;await this.loadBaseData();this.msg='Подписка продлена на '+days+' дней';}
        catch(e){this.msg='Ошибка продления: '+(e.message||e);}
        finally{this.busy=false;}
      },
      ...
      savePlan:async function(plan){
        ...
        this.syncPlanPrice(plan);syncAIState(plan);this.busy=true;
        try{
          var features=Array.isArray(plan.features)?plan.features.slice():[];
          features=features.filter(function(f){return String(f).toLowerCase().indexOf('ии-')!==0;});
          AI_FEATURES.forEach(function(x){if(plan.ai_features[x[0]])features.push(x[1]);});
          var payload={id:id,name:name,price:Number(plan.price)||0,base_price:Number(plan.base_price)||0,ai_enabled:plan.ai_enabled===true,ai_addon_price:Math.max(0,Number(plan.ai_addon_price)||0),ai_features:plan.ai_features,period:plan.period||'month',features:features,max_products:Math.max(1,Number(plan.max_products)||1),max_cooks:Math.max(1,Number(plan.max_cooks)||1),max_venues:Math.max(1,Number(plan.max_venues)||1),max_managers:Math.max(1,Number(plan.max_managers)||1),max_couriers:Math.max(1,Number(plan.max_couriers)||1),max_waiters:Math.max(1,Number(plan.max_waiters)||1),is_active:plan.is_active!==false,is_public:plan.is_public!==false,sort_order:Number(plan.sort_order)||0};
          ...
          var r=plan.__draft?await db.from('plans').insert(payload):await db.from('plans').update(payload).eq('id',plan.id);
          if(r.error)throw r.error;
          delete plan.__draft;plan.id=id;this.msg='Тариф «'+name+'» сохранён';await this.loadBaseData();
        }catch(e){this.msg='Ошибка сохранения тарифа: '+(e.message||e);}
        finally{this.busy=false;}
      },
      togglePlanVisibility:async function(plan){
        if(!plan||!plan.id)return;var next=plan.is_public===false;this.busy=true;
        try{var r=await db.from('plans').update({is_public:next}).eq('id',plan.id);if(r.error)throw r.error;plan.is_public=next;this.msg=next?'Тариф «'+plan.name+'» теперь видим управляющим':'Тариф «'+plan.name+'» скрыт от управляющих';}
        catch(e){this.msg='Ошибка изменения видимости: '+(e.message||e);}
        finally{this.busy=false;}
      },
      delPlan:async function(plan){
        ...
        this.busy=true;
        try{var r=await db.from('plans').delete().eq('id',plan.id);if(r.error)throw r.error;this.plans=this.plans.filter(function(p){return p.id!==plan.id;});this.msg='Тариф удалён';}
        catch(e){this.msg='Ошибка удаления тарифа: '+(e.message||e);}
        finally{this.busy=false;}
      }
    }
  };
