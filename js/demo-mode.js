...
// Intercept REST RPC calls too. Some staff pages keep a direct Supabase
// client reference and therefore bypass window.db.rpc. This interception is
// enabled only in demo mode and never runs in production.
(function patchSupabaseFetch(){
  if(window.__demoFetchPatched || typeof window.fetch !== 'function') return;
  window.__demoFetchPatched = true;
  var nativeFetch = window.fetch.bind(window);
  window.fetch = function(input, init){
    try{
      var url = typeof input === 'string' ? input : (input && input.url) || '';
      var m = String(url).match(/\/rest\/v1\/rpc\/([^/?#]+)/i);
      if(m){
        var rpcName = decodeURIComponent(m[1]);
        if(/^(open_staff_shift|current_staff_shift|get_staff_shift|check_staff_shift|staff_shift_status|close_staff_shift|open_shift|current_shift|check_shift)$/i.test(rpcName)){
          var isClose = /^close_/i.test(rpcName) || /^close_shift$/i.test(rpcName);
          var payload = isClose
            ? {ok:true,open:false,is_open:false,shift_open:false,status:'closed'}
            : {ok:true,open:true,is_open:true,shift_open:true,status:'open',id:'demo-shift',shift_id:'demo-shift',staff_id:'demo-staff',venue_id:D.venue.id,opened_at:new Date().toISOString()};
          return Promise.resolve(new Response(JSON.stringify(payload),{status:200,headers:{'Content-Type':'application/json'}}));
        }
      }
    }catch(e){ console.warn('[demo] fetch RPC interception:',e); }
    return nativeFetch(input,init);
  };
})();

function patchDb(){
  if(!window.db || !window.db.rpc || window.__demoDbPatched) return;
  ...
  window.db.rpc = function(name,args){
    if(/shift/i.test(String(name))){
      ...
      if(/close/i.test(String(name))) return Promise.resolve({data:{ok:true,open:false,is_open:false,shift_open:false,status:'closed'},error:null});
  ...
  window.db.from=function(table){
    ...
    var chain={select:function(){return chain;},insert:function(v){return Promise.resolve({data:Array.isArray(v)?v:[v],error:null});},update:function(v){return Promise.resolve({data:v,error:null});},delete:function(){return chain;},eq:function(){return chain;},in:function(){return chain;},order:function(){return chain;},limit:function(){return chain;},maybeSingle:function(){return Promise.resolve(single());},single:function(){return Promise.resolve(single());},then:function(a,b){return Promise.resolve(pick()).then(a,b);},catch:function(a){return Promise.resolve(pick()).catch(a);}};
    return chain;
  };
}

function blockRedirects(){
  if(typeof window.safeRedirect==='function'&&!window.__demoSafePatched){window.__demoSafePatched=true;window.safeRedirect=function(){};}
  if(typeof window.requireAuth==='function'&&!window.__demoAuthPatched){window.__demoAuthPatched=true;window.requireAuth=function(){return Promise.resolve(D.profile);};}
  if(typeof window.logout==='function'&&!window.__demoLogoutPatched){window.__demoLogoutPatched=true;window.logout=function(){localStorage.removeItem('qr_demo_mode');location.href='index.html';};}
  if(!window.__demoHistoryPatched){window.__demoHistoryPatched=true;try{var origPush=history.pushState.bind(history),origReplace=history.replaceState.bind(history);history.pushState=function(state,title,url){if(url&&String(url).indexOf('index.html')!==-1)return;return origPush(state,title,url);};history.replaceState=function(state,title,url){if(url&&String(url).indexOf('index.html')!==-1)return;return origReplace(state,title,url);};}catch(e){}}
}
...
function start(){
  ...
  showBanner();
  var tries=0;
  var mainTimer=setInterval(function(){patchDb();blockRedirects();forceFillVue();if(++tries>100)clearInterval(mainTimer);},200);
  autoStaffLogin();
}
