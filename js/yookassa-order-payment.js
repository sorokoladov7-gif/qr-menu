(function(){
  ...
  if(/[?&]demo=1(?:&|$)/i.test(location.search))return;
  ...
  window.db.rpc=function(name,args,options){
    ...
    return Promise.resolve(promise).then(async function(result){
      ...
      try{
        var response=await fetch('/api/payments/yookassa/create-order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({order_id:order.id})});
        ...
        var root=document.getElementById('app');var vm=root&&root.__vue_app__&&root.__vue_app__._instance?root.__vue_app__._instance.proxy:null;
        if(vm)vm.msg='Заказ создан, но оплату по СБП не удалось запустить: '+(e.message||'ошибка');
        else alert('Заказ создан, но оплату по СБП не удалось запустить. Попробуйте открыть оплату ещё раз.');
      }
      return result;
    });
  };
