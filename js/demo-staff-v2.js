...
var orders=[];try{orders=JSON.parse(JSON.stringify(D.orders||[]));}catch(e){orders=[];}
...
function transition(id,next){
 ...
 /* Different staff pages use different names for the same demo workflow. */
 if(['taken','accepted','cooking','preparing','in_progress','processing'].indexOf(n)>=0){if(['completed','issued','delivered','cancelled','canceled'].indexOf(s)>=0)return null;setStatus(o,n);}
 else if(['ready','prepared','done'].indexOf(n)>=0){if(['completed','issued','delivered','cancelled','canceled'].indexOf(s)>=0)return null;setStatus(o,n);}
 else if(['completed','issued','delivered','given','served'].indexOf(n)>=0){if(['cancelled','canceled'].indexOf(s)>=0)return null;setStatus(o,n);}
 else return null;
 put('qr_demo_orders',orders);return o;
}
function installUi(){
 ...
 function removeDemoBanners(){document.querySelectorAll('#qr-demo-readonly,.qr-demo-readonly-banner,.demo-banner,.demo-mode-banner').forEach(function(e){e.remove();});var all=document.querySelectorAll('body *');for(var i=0;i<all.length;i++){var t=((all[i].textContent||'')+'').trim();if(/ДЕМО[- ]?РЕЖИМ|ДЕМО\s*[·•]/i.test(t)&&all[i].children.length===0)all[i].remove();}}
 removeDemoBanners();var mo=new MutationObserver(removeDemoBanners);mo.observe(document.body,{childList:true,subtree:true});
}
function install(){
 ...
 window.db.rpc=function(name,args){args=args||{};
  ...
  if(name==='staff_update_order')return window.__qrDemoStaffUpdateOrder(args);
  if(name==='staff_update_order_status')return window.__qrDemoStaffUpdateOrder(args);
