/* QR Menu — manager website import flow v1. */
(function(){
  'use strict';
  if(window.__QR_MANAGER_SITE_IMPORT_V1__) return;
  window.__QR_MANAGER_SITE_IMPORT_V1__=true;

  var MODAL_ID='qr-manager-create-modal-v9';
  var state={mode:'template',data:null,busy:false};
  function esc(v){return String(v==null?'':v).replace(/[&<>\"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c];});}
  function slugify(v){return String(v||'').toLowerCase().trim().replace(/[^a-z0-9а-яё\s_-]/gi,'').replace(/\s+/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'').slice(0,80);}
  ...
  function install(modal){
    if(!modal||modal.dataset.siteImportInstalled==='1')return;
    modal.dataset.siteImportInstalled='1';
    ...
    switcher.id='qr-site-import-switcher-v1';
    ...
    content.insertBefore(switcher,templateLabel||templateGrid);

    var panel=document.createElement('div');
    panel.id='qr-site-import-panel-v1';
    ...
    panel.innerHTML='<div style="font-weight:800;margin-bottom:6px">🌐 Импорт существующего заведения</div><div style="color:#9ca3af;font-size:12px;margin-bottom:10px">Укажите сайт. Система попробует найти название, адрес, телефон, логотип, часы работы и меню.</div><div style="display:flex;gap:8px;flex-wrap:wrap"><input id="qr-site-url-v1" type="url" placeholder="https://example.ru" style="flex:1;min-width:220px;background:#0f172a;color:#fff;border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:12px;box-sizing:border-box"><button id="qr-site-find-v1" type="button" style="background:#2563eb;color:#fff;border:0;border-radius:10px;padding:12px 16px;font-weight:800;cursor:pointer">🔍 Найти информацию</button></div><div id="qr-site-preview-v1" style="margin-top:12px"></div>';
    content.insertBefore(panel,switcher.nextSibling);
    ...
    function setMode(mode){
      ...
      var site=mode==='site';
      panel.style.display=site?'block':'none';
      if(templateLabel)templateLabel.style.display=site?'none':'';
      templateGrid.style.display=site?'none':'grid';
      if(templatePreview)templatePreview.style.display=site?'none':'';
      submit.textContent=site?'Создать импортированное заведение':'Создать заведение';
      if(site){name.placeholder='Название будет найдено автоматически';slug.placeholder='Код создастся автоматически';}
    }
    modeButtons.forEach(function(b){b.onclick=function(){setMode(b.dataset.mode);};});

    panel.querySelector('#qr-site-find-v1').onclick=async function(){
      if(state.busy)return;
      var input=panel.querySelector('#qr-site-url-v1');var url=input.value.trim();hideError(modal);
      if(!url){showError(modal,'Введите адрес сайта заведения');return;}
      if(!/^https?:\/\//i.test(url))url='https://'+url;
      state.busy=true;this.disabled=true;this.textContent='Анализирую...';
      try{
        var r=await fetch('/api/import-site?url='+encodeURIComponent(url),{credentials:'same-origin'});
        var data=await r.json().catch(function(){return{};});
        if(!r.ok)throw new Error(data.message||data.error||'Не удалось получить сайт');
        state.data=data;
        var v=data.venue||{}, items=Array.isArray(data.products)?data.products:[];
        name.value=v.name||'';slug.value=slugify(v.name||new URL(url).hostname.split('.')[0]);
        var preview=panel.querySelector('#qr-site-preview-v1');
        preview.innerHTML='<div style="border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px;background:rgba(15,23,42,.65)"><div style="display:flex;gap:12px;align-items:center">'+(v.logo_url?'<img src="'+esc(v.logo_url)+'" style="width:52px;height:52px;border-radius:12px;object-fit:cover" onerror="this.style.display=\'none\'">':'<div style="font-size:34px">🏪</div>')+'<div><b style="font-size:17px">'+esc(v.name||'Заведение')+'</b><div style="font-size:12px;color:#94a3b8">'+esc(v.address||'Адрес не найден')+'</div></div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:8px;margin-top:12px;font-size:12px;color:#cbd5e1"><div>☎ '+esc(v.phone||'не найден')+'</div><div>🌐 '+esc(v.website_url||url)+'</div><div>🍽️ '+items.length+' позиций</div></div>'+ (v.description?'<div style="margin-top:10px;color:#94a3b8;font-size:12px">'+esc(v.description)+'</div>':'')+'<div style="margin-top:10px;color:#6ee7b7;font-size:12px">✓ Данные готовы. Проверьте название и код, затем нажмите создание.</div></div>';
      }catch(e){showError(modal,'Ошибка импорта: '+(e.message||e));}
    ...
    submit.addEventListener('click',async function(e){
      ...
      var v=state.data.venue||{}, items=Array.isArray(state.data.products)?state.data.products:[];
      var venueName=(name.value.trim()||v.name||'').trim();
      var venueSlug=slugify(slug.value.trim()||v.name||'');
      if(!venueName){showError(modal,'Не найдено название заведения');return;}
      if(!venueSlug){showError(modal,'Не удалось сформировать код заведения');return;}
      state.busy=true;submit.disabled=true;submit.textContent='Создаю...';
      try{
        var u=await db.auth.getUser();var uid=u&&u.data&&u.data.user&&u.data.user.id;if(!uid)throw new Error('Сессия управляющего не найдена');
        var sr=await db.from('subscriptions').select('plan_id,current_period_end,status').eq('manager_id',uid).order('created_at',{ascending:false}).limit(1).maybeSingle();
        if(sr.error)throw sr.error;if(!sr.data)throw new Error('Подписка не найдена');
        if(['active','trialing'].indexOf(sr.data.status)===-1)throw new Error('Подписка не активна');
        var pr=await db.from('plans').select('id,max_products').eq('id',sr.data.plan_id).maybeSingle();
        if(pr.error)throw pr.error;
        if(pr.data&&pr.data.max_products&&items.length>Number(pr.data.max_products))throw new Error('На выбранном тарифе лимит '+pr.data.max_products+' позиций, а найдено '+items.length+'.');
        ...
        var rr=await db.rpc('manager_import_venue',{p_name:venueName,p_slug:venueSlug,p_plan:sr.data.plan_id,p_subscription_end:sr.data.current_period_end,p_address:v.address||null,p_phone:v.phone||null,p_website_url:v.website_url||panel.querySelector('#qr-site-url-v1').value.trim(),p_description:v.description||null,p_logo_url:v.logo_url||null,p_opening_hours:v.opening_hours||null,p_products:payload});
        if(rr.error)throw rr.error;
        alert('Заведение «'+venueName+'» создано из сайта. Импортировано позиций: '+(rr.data&&rr.data.products_count!=null?rr.data.products_count:payload.length)+'.');
        location.reload();
