/* QR Menu — manager website import flow v2. */
(function(){
  'use strict';
  if(window.__QR_MANAGER_SITE_IMPORT_V2__) return;
  window.__QR_MANAGER_SITE_IMPORT_V2__=true;
  ...
  function install(modal){
    if(!modal||modal.dataset.siteImportInstalled==='2')return;
    modal.dataset.siteImportInstalled='2';
    ...
    if(!templateGrid||!name||!slug||!submit){modal.dataset.siteImportInstalled='';return;}
    ...
    panel.querySelector('#qr-site-find-v2').onclick=async function(){
      ...
      var input=panel.querySelector('#qr-site-url-v2');var status=panel.querySelector('#qr-site-status-v2');var preview=panel.querySelector('#qr-site-preview-v2');var url=input.value.trim();hideError(modal);
      ...
      try{
        ...
        status.textContent='✓ Сайт обработан'+(data.meta&&data.meta.source==='reader_fallback'?' через резервный режим':'')+'. Проверьте найденные данные.';
    ...
    submit.addEventListener('click',async function(e){
      ...
      var v=state.data.venue||{},items=Array.isArray(state.data.products)?state.data.products:[];var venueName=(name.value.trim()||v.name||'').trim();var venueSlug=slugify(slug.value.trim()||v.name||'');
