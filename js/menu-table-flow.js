...
function install(x,t){
 ...
 if(typeof oldCheckout==='function'){
  x.checkout=async function(){
   ...
   self.msg='';var phone=self.norm(self.form.phone);
   ...
   try{
    ...
    var addons=[];self.cart.forEach(function(i){(i.addons||[]).forEach(function(a){var q=Number(a.qty)||0;if(q>0)addons.push({id:a.id,qty:q,item_name:i.name});});});
    var r=await db.rpc('create_public_order',{p_venue_id:self.venue.id,p_order_type:self.form.type,p_customer_name:self.form.name||null,p_customer_phone:phone,p_delivery_address:self.form.type==='delivery'?self.form.address:null,p_comment:self.form.comment||null,p_payment_method:self.form.pay,p_items:items,p_addons:addons,p_total_price:self.cartTotal,p_table_token:tt});
