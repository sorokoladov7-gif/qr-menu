...
(function(){
 ...
 if(!type)return;
 const real=window.db, secureKey='qr_staff_'+type, legacyKey=type+'_session';
 let slug='',lastToken='';
 ...
 function chain(table){
   ...
   const api={select:function(){state.action='select';return api},update:function(v){state.action='update';state.payload=v;return api},eq:function(k,v){state.filters[k]=v;if(k==='slug')slug=v;return api},in:function(k,v){state.filters[k]={in:v};return api},order:function(){return api},limit:function(){return api},maybeSingle:function(){return execute(true)},then:function(resolve,reject){return execute(false).then(resolve,reject)},catch:function(reject){return execute(false).catch(reject)}};
   async function execute(single){
     const saved=JSON.parse(localStorage.getItem(secureKey)||'null');
     ...
     if(['cooks','couriers','waiters'].includes(table)&&state.action==='select'){
       ...
       lastToken=data.token; originalSet(secureKey,JSON.stringify(data));
     ...
     if(table==='orders'&&state.action==='select'){
       ...
       const history=!!(state.filters.courier_name||state.filters.waiter_name||state.filters.cook_name);
       const fn=history?'staff_history_json':'staff_orders_json';
       ...
       if(state.filters.cook_name)rows=rows.filter(o=>o.cook_name===state.filters.cook_name);
       return {data:rows,error};
     }
     if(table==='orders'&&state.action==='update'){
       if(!saved?.token)return {data:null,error:new Error('invalid_session')};
       const {data,error}=await real.rpc('staff_update_order',{p_token:saved.token,p_order_id:state.filters.id,p_status:state.payload?.status});
       return {data,error};
     }
