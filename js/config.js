...
function makeChain(table){
  ...
  async function execute(single){
    ...
    if(isStaff && ['cooks','waiters','couriers'].indexOf(table)>=0 && state.action==='select' && state.filters.venue_id && state.filters.pin){
      ...
      if(r2.data && r2.data.error) return { data:null, error:{message: r2.data.error} };
      if(r2.error) return { data:null, error:r2.error };
      rememberStaffLogin(type, r2.data);
      return { data: r2.data ? {id:r2.data.staffId, name:r2.data.staffName, venue_id:r2.data.venueId, token:r2.data.token} : null, error: null };
    }
    ...
    if(isStaff && table==='orders' && state.action==='update'){
      ...
      var r4 = await oldRpc('staff_update_order', {p_token:token, p_order_id:state.filters.id, p_status:state.payload&&state.payload.status});
    ...
    if(table==='orders' && state.action==='update' && isMenu){
      return oldRpc('customer_change_order_status', {p_order_id:state.filters.id, p_customer_phone:localStorage.getItem('last_phone')||'', p_status:state.payload&&state.payload.status});
    }
...
window.db = {from:function(table){if(isStaff) return makeChain(table);if(isMenu && table==='orders') return makeChain(table);return oldFrom(table);},rpc:rpc,auth:real.auth,storage:real.storage};
...
if(isMenu){bootQrTable();var n=0;var timer=setInterval(function(){renderCustomerTable();if(++n>30)clearInterval(timer)},500);}
...
function installTableControl(){
  if(!isStaff||staffType==='courier') return;
  ...
  btn.style.cssText='border:1px solid rgba(129,140,248,.35);border-radius:12px;padding:10px 22px;background:linear-gradient(135deg,#4f46e5,#6366f1);color:#fff;font-weight:800;box-shadow:0 8px 24px rgba(79,70,229,.25);cursor:pointer;min-width:170px;display:inline-flex;align-items:center;justify-content:center;';
  var target=document.querySelector('.tabs');
  if(!target && staffType==='cook') target=document.querySelector('.cols');
  if(!target){setTimeout(installTableControl,100);return;}
  var wrap=document.createElement('div');
  wrap.id='staff-table-inline';
  wrap.style.cssText='display:flex;justify-content:center;align-items:center;margin:0 auto 14px;width:100%;';
...
async function showStaffTables(){
  ...
  if(r.error){
    ...
    if(msg.indexOf('does not exist')!==-1||msg.indexOf('403')!==-1||msg.indexOf('not found')!==-1||msg.indexOf('invalid_session')!==-1) alert('🪑 '+(r.error.message||'Не удалось загрузить столы')+'\n\nПопробуйте перезайти (Выйти → Войти).');
    else alert('Не удалось загрузить столы: '+(r.error.message||r.error));
    return;
  }
  var payload=r.data||{};
  ...
  modal=document.createElement('div');modal.id='staff-table-modal';modal.style.cssText='position:fixed;inset:0;z-index:99999;background:rgba(5,10,20,.82);backdrop-filter:blur(8px);padding:20px;overflow:auto';
  var box=document.createElement('div');box.style.cssText='max-width:900px;margin:20px auto;background:#111827;border:1px solid rgba(255,255,255,.12);border-radius:20px;padding:20px;color:#fff';
  var head=document.createElement('div');head.style.cssText='display:flex;justify-content:space-between;align-items:center;margin-bottom:16px';head.innerHTML='<h2 style="margin:0">🪑 Столы</h2><button id="staff-table-close" style="border:0;border-radius:10px;padding:9px 12px;background:rgba(255,255,255,.1);color:#fff;cursor:pointer">✕</button>';box.appendChild(head);
  var info=document.createElement('div');info.style.cssText='color:#94a3b8;font-size:13px;margin-bottom:14px';info.textContent=staffType==='cook'?(canControl?'Официанта нет — столами управляет повар':'Есть официант — управление столами передано официанту'):'Официант управляет посадкой и освобождением столов';box.appendChild(info);
  var grid=document.createElement('div');grid.style.cssText='display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:12px';
  tables.forEach(function(t){
    var occupied=t.occupancy_status==='occupied';
    var card=document.createElement('div');card.style.cssText='border:1px solid '+(occupied?'rgba(251,191,36,.45)':'rgba(52,211,153,.35)')+';border-radius:14px;padding:14px;background:rgba(255,255,255,.03)';
    ...
    if(t.session) html+='<div style="font-size:12px;color:#94a3b8">Заказов: '+(t.session.order_count||0)+' · '+Number(t.session.total_price||0).toLocaleString('ru-RU')+' ₽</div>';
    if(canControl) html+='<button class="staff-seat-btn" style="width:100%;margin-top:10px;border:0;border-radius:10px;padding:9px;background:'+(occupied?'#7f1d1d':'#047857')+';color:#fff;font-weight:700;cursor:pointer">'+actionText+'</button>';
    else html+='<div style="margin-top:10px;color:#94a3b8;font-size:12px">Только просмотр</div>';
    card.innerHTML=html;
    var action=card.querySelector('.staff-seat-btn');
    if(action) action.onclick=async function(){action.disabled=true;var rr;if(occupied) rr=await oldRpc(staffType==='cook'?'cook_release_table':'waiter_release_table',{p_token:token,p_table_id:t.id});else rr=await oldRpc(staffType==='cook'?'cook_start_table_session':'waiter_start_table_session',{p_token:token,p_table_id:t.id});if(rr.error){alert(rr.error.message||'Операция не выполнена');action.disabled=false;return}modal.remove();showStaffTables();};
    grid.appendChild(card);
  });
  box.appendChild(grid);modal.appendChild(box);document.body.appendChild(modal);modal.onclick=function(e){if(e.target===modal)modal.remove()};box.querySelector('#staff-table-close').onclick=function(){modal.remove()};
}

if(isStaff){
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',function(){installTableControl();loadStaffOrders();setInterval(loadStaffOrders,3000)});
  else {installTableControl();loadStaffOrders();setInterval(loadStaffOrders,3000);}
}
})();
