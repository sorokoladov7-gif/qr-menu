...
(function(){
  window.StaffAPI = {
    ...
    orders: async function(token){
      const {data,error}=await db.rpc('staff_orders_json',{p_token:token});
      if(error) throw error; return data||[];
    },
    ...
    history: async function(token){
      const {data,error}=await db.rpc('staff_history_json',{p_token:token});
      if(error) throw error; return data||[];
    },
    logout: async function(token){
      try{await db.rpc('staff_logout',{p_token:token});}catch(e){}
    }
  };
})();
