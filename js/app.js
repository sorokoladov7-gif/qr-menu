...
(function(){
  ...
  function start(){if(started)return;started=true;loadSiteImport();var attempts=0;var timer=setInterval(function(){attempts++;loadSiteImport();if(window.QRManagerSiteImport||attempts>=60)clearInterval(timer);},500);}
...
(function(){
  ...
  function start(){sync();setInterval(sync,250);} if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start);else start();
})();