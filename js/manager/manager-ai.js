...
(function(){
  ...
  /* Single frontend entitlement source: manager-app.js.
     Qrchick never reloads subscriptions/plans itself. */
  ...
  function install(){
    ...
    root.querySelector('#qrchick-manager-fab').onclick=function(){state.open=!state.open;if(state.open&&!state.history.length)add('assistant','Я Qrchick — ИИ-помощник управляющего. Могу выполнять доступные тебе операции с меню, заказами, персоналом, рецептами, залом, настройками и аналитикой.');render();};
  ...
  function syncView(){state.entitlementsReady=!!vm();render();}
  window.addEventListener('qr-manager-vue-ready',function(){setTimeout(function(){install();syncView();},100);});
  window.addEventListener('qr-manager-subscription-ready',function(){setTimeout(syncView,50);});
  window.addEventListener('qr-manager-ai-entitlements-updated',function(){syncView();});
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(function(){install();syncView();},250);},{once:true});else setTimeout(function(){install();syncView();},250);
})();