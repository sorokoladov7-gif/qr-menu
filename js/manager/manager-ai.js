...
(function(){
  ...
  /* Canonical entitlement source: the manager-owned subscription RPC is
     SECURITY DEFINER, so UI entitlement does not depend on a fragile RLS
     SELECT. The plan itself is then refreshed from public.plans. */
  function syncEntitlements(){
    ...
    return db.rpc('manager_ensure_subscription').then(function(r){
      ...
      return db.from('plans').select('id,name,ai_enabled,ai_features,max_venues,max_products,is_active').order('price');
    }).then(function(r){
      if(r&&r.error)throw r.error;
      if(r&&Array.isArray(r.data))v.plans=r.data;
  ...
  function allowed(feature){
    ...
    try{
      ...
      var s=v.managerSubscription;
      if(s&&['active','trialing'].indexOf(s.status)!==-1&&s.current_period_end&&new Date(s.current_period_end)>=new Date()){
        if(s.status==='trialing')return true;
        var p=(v.plans||[]).find(function(x){return x.id===s.plan_id;});
        return !!(p&&p.ai_enabled===true&&p.ai_features&&p.ai_features[feature]===true);
      }
      return false;
