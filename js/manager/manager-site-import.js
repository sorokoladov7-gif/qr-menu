...
(function(){
  ...
  async function startFile(file,v){if(!file)return;if(file.size>MAX_FILE)throw new Error('Файл превышает лимит 10 МБ.');var mime=String(file.type||'').toLowerCase();var ok=(mime==='application/pdf'||mime==='image/jpeg'||mime==='image/png'||mime==='image/webp'||/\.pdf$/i.test(file.name)||/\.(jpe?g|png|webp)$/i.test(file.name));if(!ok)throw new Error('Поддерживаются только PDF, JPG, PNG и WEBP.');var temp=await uploadTemp(file,v);try{setStatus(mime==='application/pdf'||/\.pdf$/i.test(file.name)?'🤖 Gemini анализирует весь PDF целиком…':'🤖 Gemini анализирует изображение…');var data=await request({source:'file',language:'ru',file:{url:temp.url,temp_path:temp.path,name:file.name,mime:file.type,size:file.size}});v.importItems=normalizeMenu(data);v.importWarnings=Array.isArray(data.warnings)?data.warnings:[];render(v.importItems,v.importWarnings);setStatus('✓ Gemini завершил анализ: '+v.importItems.length+' позиций'+(v.importWarnings.length?' · есть предупреждения':''));}finally{await deleteTemp(temp.path);}}
  async function startUrl(url,v){url=clean(url);if(!/^https?:\/\//i.test(url))throw new Error('Укажите корректную ссылку http/https.');setStatus('🤖 Gemini анализирует сайт…');var data=await request({source:'url',language:'ru',url:url});v.importItems=normalizeMenu(data);v.importWarnings=Array.isArray(data.warnings)?data.warnings:[];render(v.importItems,v.importWarnings);setStatus(v.importItems.length?'✓ Gemini завершил анализ сайта: '+v.importItems.length+' позиций':'⚠ Gemini не нашёл меню. Укажите прямую ссылку на страницу меню.',!v.importItems.length);if(data.menu&&data.menu.venue_name)v.importVenue={name:data.menu.venue_name};}
