...
(function(){
  ...
  function watchDeliverySettingsMount(){var attempts=0;var timer=setInterval(function(){restoreDeliverySettingsMount();attempts++;if(document.querySelector('[data-qr-delivery-settings]')||attempts>=120)clearInterval(timer);},250);setTimeout(function(){clearInterval(timer);},30000);}
  ...
  function installSettingsPersistencePatch(vm){
    ...
    if(typeof originalSaveVenue==='function'){
      vm.saveVenue=function(){var self=this,venueId=self.venue&&self.venue.id;if(!venueId)return originalSaveVenue.apply(this,arguments);var f=self.vform||{},lat=Number(f.latitude),lng=Number(f.longitude),hasLat=Number.isFinite(lat),hasLng=Number.isFinite(lng);var payload={p_venue_id:venueId,p_address:String(f.address==null?'':f.address).trim()||null,p_latitude:hasLat?lat:null,p_longitude:hasLng?lng:null,p_delivery_enabled:typeof f.delivery_enabled==='boolean'?f.delivery_enabled:null,p_delivery_min_order:Math.max(0,Number(f.delivery_min_order)||0),p_delivery_min_order_free:Math.max(0,Number(f.delivery_min_order_free)||0),p_delivery_base_fee:Math.max(0,Number(f.delivery_base_fee)||0),p_delivery_rate_per_km:Math.max(0,Number(f.delivery_rate_per_km)||0),p_delivery_max_km:Math.max(0,Number(f.delivery_max_km)||0)};return db.rpc('manager_save_venue_settings',payload).then(function(r){if(r.error)throw r.error;var v=r.data&&r.data.venue?r.data.venue:null;if(v)self.venue=v;return originalSaveVenue.apply(self,arguments);}).catch(function(e){console.error('[Manager] manager_save_venue_settings:',e);self.showToast('Ошибка сохранения настроек заведения: '+(e.message||String(e)),'error');throw e;});
    }
  }
