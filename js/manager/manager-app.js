...
(function(){
  ...
  appComputed.aiEnabled=function(){
    if(this.profile&&this.profile.role==='admin')return true;
    var sub=this.managerSubscription;
    var active=!!(sub&&['trialing','active'].includes(sub.status)&&sub.current_period_end&&new Date(sub.current_period_end)>=new Date());
    return active&&!!(this.currentPlan&&this.currentPlan.ai_enabled===true);
  };
  ...
  function startManagerAIGate(vm){
    ...
    var root=document.getElementById('app');if(root)observer.observe(root,{subtree:true,childList:true});
  ...
  function installVenueNameSuggestions(vm){
    if(!vm)return;
    var root=document.getElementById('app');var modal=root&&root.querySelector('.modal');if(!modal)return;
    ...
    function hide(){box.style.display='none';box.innerHTML='';}function choose(text){input.value=text;input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));try{vm.newVenueForm.name=text;}catch(e){}hide();input.focus();}
    function render(){var items=buildSuggestions(input.value||'');if(!items.length){hide();return;}box.innerHTML='';items.forEach(function(item){var row=document.createElement('button');row.type='button';row.textContent=item.text;row.style.cssText='display:block;width:100%;text-align:left;border:0;background:transparent;color:#e5e7eb;padding:9px 10px;border-radius:7px;cursor:pointer;font-size:13px;';row.addEventListener('mouseenter',function(){row.style.background='rgba(99,102,241,.12)';});row.addEventListener('mouseleave',function(){row.style.background='transparent';});row.addEventListener('mousedown',function(e){e.preventDefault();choose(item.text);});box.appendChild(row);});box.style.display='block';}
    input.addEventListener('input',render);input.addEventListener('focus',render);input.addEventListener('blur',function(){setTimeout(hide,140);});document.addEventListener('keydown',function(e){if(e.key==='Escape')hide();});vm.$nextTick(render);
  }

  var basePrepareCreateVenueModal=appMethods.prepareCreateVenueModal;
  if(typeof basePrepareCreateVenueModal==='function'){appMethods.prepareCreateVenueModal=function(){var self=this;return Promise.resolve(basePrepareCreateVenueModal.apply(this,arguments)).then(function(result){self.$nextTick(function(){installVenueNameSuggestions(self);});return result;});};}
  ...
  function loadInstruction(){
    ...
    var script=document.createElement('script');script.src='/js/manager-instruction-tab-v2.js?v=6';script.async=false;script.setAttribute('data-qr-manager-instruction','v6');script.onload=function(){window.__QR_MANAGER_INSTRUCTION_LOADING__=false;};script.onerror=function(){window.__QR_MANAGER_INSTRUCTION_LOADING__=false;};document.head.appendChild(script);
  }
  ...
  function boot(){
    ...
    var app=window.Vue.createApp(window.__QR_MANAGER_APP_MIXIN__);
    app.config.errorHandler=function(err,instance,info){console.error('[QR Manager Vue]',err,info);};
    app.mount(root);
    window.__managerVue=app._instance&&app._instance.proxy?app._instance.proxy:null;
    try{window.dispatchEvent(new CustomEvent('qr-manager-vue-ready',{detail:{vm:window.__managerVue}}));}catch(e){}
  }
