...
(function(){
  'use strict';
  if(window.__QR_MANAGER_CREATE_FLOW_V2__) return;
  window.__QR_MANAGER_CREATE_FLOW_V2__=true;
  ...
  function normalizePlans(rows){
    return (rows||[]).filter(function(p){ return p && p.is_active !== false; });
  }
    ...
    plans.forEach(function(plan){
      ...
      card.onclick=async function(){
        ...
        try{
          ...
          var result=await db.from('subscriptions').upsert({
            manager_id:userId,
            venue_id:null,
            plan_id:plan.id,
            status:'trialing',
  ...
  async function handleCreate(target){
    ...
    var sub=await db.from('subscriptions')
      .select('id,manager_id,venue_id,plan_id,status,current_period_end')
    ...
    var plansResult=await db.from('plans')
      .select('id,name,price,max_venues,max_products,max_cooks,max_couriers,max_waiters,is_active,sort_order')
      .eq('is_active',true)
      .order('sort_order');
    if(plansResult.error) throw plansResult.error;
    var plans=normalizePlans(plansResult.data);
    if(!plans.length) throw new Error('Нет доступных тарифов');
    showPlans(p,user.id,plans);
  }

  /* Delegated capture listener: Vue never receives the create click before this runs. */
  document.addEventListener('click',function(ev){
    ...
    Promise.resolve(handleCreate(button))
      .catch(function(e){
