...
function install(){
 ...
 window.db.rpc=function(name,args){
   ...
   if(name==='staff_update_order'){
     var found=(D.orders||[]).find(function(o){return o.id===args.p_order_id;});
     if(found)found.status=args.p_status;
     return Promise.resolve({data:found||{id:args.p_order_id,status:args.p_status},error:null});
   }
 ...
 // Existing staff pages restore their session from sessionStorage. If the page
 // has already mounted, force the same login method once the patched RPC exists.
 var tries=0;
 var timer=setInterval(function(){
   tries++;
   var root=document.getElementById('app');
   var inst=root&&root.__vue_app__&&root.__vue_app__._instance;
   var vm=inst&&inst.proxy;
   if(vm){
     if(!vm.session&&typeof vm.login==='function'){
       vm.form=vm.form||{};vm.form.slug=D.venue.slug;vm.form.pin=role==='cook'?'1234':role==='courier'?'1111':'2222';
       try{vm.login();}catch(e){}
     }
     if(vm.session&&timer){clearInterval(timer);timer=null;}
   }
   if(tries>100&&timer){clearInterval(timer);timer=null;}
 },100);
}
install();
})();
