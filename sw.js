...
async function enhanceHtml(r) {
  ...
  if(pathname.endsWith('/manager.html') || pathname==='/manager.html'){
    extra+=`<script>
(function(){
'use strict';
if(window.__QR_MANAGER_CREATE_BUTTON_FIX_V13__)return;
window.__QR_MANAGER_CREATE_BUTTON_FIX_V13__=true;
function getButtons(){var root=document.getElementById('app')||document.body;return Array.prototype.slice.call(root.querySelectorAll('button')).filter(function(b){var t=(b.textContent||'').replace(/\s+/g,' ').trim();return /^\+?\s*Создать$/i.test(t)||/^\+?\s*Создать\s+заведение$/i.test(t);});}
async function getLimit(){
  if(typeof db==='undefined'||!db.auth||typeof db.from!=='function')return null;
  var u=await db.auth.getUser(),uid=u&&u.data&&u.data.user&&u.data.user.id;if(!uid)return null;
  var s=await db.from('subscriptions').select('id,plan_id,status,current_period_end,created_at').eq('manager_id',uid).is('venue_id',null).in('status',['trialing','active']).gte('current_period_end',new Date().toISOString()).order('created_at',{ascending:false}).limit(1).maybeSingle();
...
async function sync(){
  try{
    ...
    getButtons().forEach(function(b){
      ...
      b.title=state.allowed?'Создать заведение. Доступно: '+Math.max(0,state.limit-state.count)+' из '+state.limit:'Достигнут лимит заведений по тарифу';
    });
  }catch(e){console.warn('[QR Manager] create button fix:',e);}
}
function loadCreateFlow(){
  if(document.querySelector('script[data-qr-manager-create-v11]'))return;
  var s=document.createElement('script');s.src='/js/manager-create-venue-flow.js?v=11';s.async=false;s.dataset.qrManagerCreateV11='1';s.onload=function(){setTimeout(sync,100);};document.head.appendChild(s);
}
function start(){
  loadCreateFlow();
  sync();
  ...
  obs.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['disabled','style']});
  setTimeout(function(){loadCreateFlow();sync();},500);
  setTimeout(function(){sync();},1500);
  setTimeout(function(){sync();},3000);
  setTimeout(function(){sync();},7000);
}
