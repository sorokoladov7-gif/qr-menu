...
(function(){
  ...
  function styles(){if(document.getElementById('qr-admin-support-style'))return;var s=document.createElement('style');s.id='qr-admin-support-style';s.textContent=''+
    ...
    '@media(max-width:800px){.qr-admin-support-panel{grid-template-columns:1fr}.qr-admin-support-list{max-height:230px;border-right:0;border-bottom:1px solid rgba(255,255,255,.08)}.qr-admin-support-messages{min-height:330px}.qr-admin-support-msg{max-width:94%}}';document.head.appendChild(s);}
  function setBadge(n){state.unreadTotal=Number(n)||0;var b=state.button&&state.button.querySelector('[data-admin-support-badge]');if(!b)return;b.textContent=String(state.unreadTotal);b.style.display=state.unreadTotal?'inline-flex':'none';}
  async function refreshUnreadBadge(){
    try{
      var r=await db.from('manager_support_messages').select('id,thread_id').eq('sender_role','manager').is('read_at',null);
      if(r.error)throw r.error;
      var map={},total=0;(r.data||[]).forEach(function(m){map[m.thread_id]=(map[m.thread_id]||0)+1;total++;});
      state.unreadByThread=map;setBadge(total);renderList();
    }catch(e){console.warn('[QR Admin Support] unread count:',e);}
  }
  async function loadThreads(){try{var r=await db.from('manager_support_threads').select('id,manager_id,venue_id,subject,status,last_message_at,created_at,updated_at').neq('status','closed').order('last_message_at',{ascending:false});if(r.error)throw r.error;var threads=r.data||[];var ids=threads.map(function(t){return t.manager_id;});var profiles={};if(ids.length){var p=await db.from('profiles').select('id,display_name,email').in('id',ids);if(p.error)throw p.error;(p.data||[]).forEach(function(x){profiles[x.id]=x;});}threads.forEach(function(t){t.manager=profiles[t.manager_id]||{};});state.threads=threads;await refreshUnreadBadge();renderList();if(state.selected&&!threads.some(function(t){return t.id===state.selected;})){state.selected=null;state.messages=[];}if(!state.selected&&threads.length)selectThread(threads[0].id);}catch(e){console.error('[QR Admin Support] threads:',e);renderError(e);}}
  async function selectThread(id){state.selected=id;renderList();state.loading=true;try{var r=await db.from('manager_support_messages').select('id,thread_id,sender_id,sender_role,message,created_at,read_at').eq('thread_id',id).order('created_at',{ascending:true});if(r.error)throw r.error;state.messages=r.data||[];var mark=await db.from('manager_support_messages').update({read_at:new Date().toISOString()}).eq('thread_id',id).eq('sender_role','manager').is('read_at',null);if(mark.error)throw mark.error;state.unreadByThread[id]=0;await refreshUnreadBadge();renderChat(true);}catch(e){renderError(e);}finally{state.loading=false;}}
  function renderList(){if(!state.panel)return;var box=state.panel.querySelector('[data-admin-support-list]');if(!box)return;box.innerHTML='<div class="qr-admin-support-list-head">Вопросы управляющих <span class="badge">'+state.threads.length+'</span>'+ (state.unreadTotal?' <span class="qr-admin-support-unread">Непрочитано: '+state.unreadTotal+'</span>':'')+'</div>'+(state.threads.length?state.threads.map(function(t){var name=t.manager.display_name||t.manager.email||'Управляющий',unread=Number(state.unreadByThread[t.id]||0);return '<button class="qr-admin-support-item '+(state.selected===t.id?'active':'')+'" data-thread="'+esc(t.id)+'"><div class="name">'+esc(name)+(unread?' <span class="qr-admin-support-unread">'+unread+'</span>':'')+'</div><div class="meta">'+esc(t.status==='in_progress'?'В работе':'Новый')+' · '+esc(fmt(t.last_message_at))+'</div><div class="preview">'+esc(t.subject||'Поддержка')+'</div></button>';}).join(''):'<div class="qr-admin-support-empty">Новых обращений нет.</div>');Array.prototype.forEach.call(box.querySelectorAll('[data-thread]'),function(b){b.addEventListener('click',function(){selectThread(b.getAttribute('data-thread'));});});}
