...
(function(){
  ...
  /* menu_import is intentionally NOT a Qrchick chat feature.
     It has its own UI/backend and its own Vercel GEMINI_API_KEY provider. */
  var FEATURES=[
    ['assistant','Qrchick'],['menu_analysis','Анализ меню'],
  ...
  /* Frontend gate is intentionally asymmetric:
     Qrchick itself only needs a signed-in manager here. The server is the
     authoritative subscription/entitlement gate for the actual AI request.
     This prevents a stale/competing Vue method from hiding Qrchick when the
     subscription or plan catalogue is still hydrating. Specialized AI tools
     remain gated by their normal feature checks. */
  function allowed(feature){
    var v=vm();
    try{
      if(!v)return false;
      if(v.profile&&v.profile.role==='admin')return true;
      if(feature==='assistant')return !!(v.profile&&v.profile.role==='manager');
      if(typeof v.hasAIFeature==='function')return v.hasAIFeature(feature)===true;
      if(typeof v.aiFeatureEnabled==='function')return v.aiFeatureEnabled(feature)===true;
      return false;
    }catch(e){
  ...
  function send(text){
    ...
    if(!allowed(feature)){
      add('assistant','Функция «'+LABELS[feature]+'» не включена в текущем тарифе.');
      render();
      return;
    }
  ...
  function renderFeatures(){
    ...
    if(b)b.querySelectorAll('[data-feature]').forEach(function(x){x.onclick=function(){var f=x.getAttribute('data-feature');if(!allowed(f)){render();return;}state.feature=f;render();};});
  ...
  function install(){
    ...
    var style=document.createElement('style');style.id='qrchick-manager-style';style.textContent='#qrchick-manager-root{position:fixed;right:18px;bottom:18px;z-index:15000;font-family:inherit;color:#fff}#qrchick-manager-fab{width:78px;height:78px;border-radius:50%;padding:0;border:2px solid rgba(62,196,255,.78);background:#06152e;box-shadow:0 0 28px rgba(24,157,255,.5);cursor:pointer;overflow:hidden}#qrchick-manager-fab img,#qrchick-manager-brand img{display:block;width:100%;height:100%;object-fit:cover}#qrchick-manager-panel{display:none;flex-direction:column;position:absolute;right:0;bottom:92px;width:min(500px,calc(100vw - 24px));height:min(760px,calc(100vh - 110px));background:rgba(5,23,53,.985);border:1px solid rgba(64,181,255,.46);border-radius:20px;box-shadow:0 24px 80px rgba(1,20,52,.55);backdrop-filter:blur(18px);overflow:hidden}.qrchick-manager-head{display:flex;justify-content:space-between;align-items:center;padding:14px 16px 10px;border-bottom:1px solid rgba(148,163,184,.12)}.qrchick-manager-brand{display:flex;align-items:center;gap:10px}.qrchick-manager-brand img{width:42px;height:42px;border-radius:12px}.qrchick-manager-brand b{display:block;font-size:15px}.qrchick-manager-brand span{display:block;color:#93c5fd;font-size:10px;margin-top:2px}.qrchick-manager-head>button{background:none;border:0;color:#94a3b8;font-size:26px;cursor:pointer}.qrchick-manager-sub{padding:10px 16px;color:#94a3b8;font-size:11px}.qrchick-manager-panel{}#qrchick-manager-features{display:flex;gap:6px;flex-wrap:wrap;padding:0 12px 10px}.qrchick-feature{border:1px solid rgba(148,163,184,.18);background:rgba(15,23,42,.45);color:#cbd5e1;border-radius:999px;padding:7px 10px;font-size:11px;cursor:pointer}.qrchick-feature.on{border-color:rgba(62,196,255,.55);color:#fff;background:rgba(24,96,155,.22)}.qrchick-feature.locked{opacity:.48;cursor:not-allowed}.qrchick-feature.locked:active{transform:none}.qrchick-feature:disabled{cursor:not-allowed}.qrchick-feature+ .qrchick-feature{}.qrchick-feature{}.qrchick-feature:hover{border-color:rgba(62,196,255,.36)}#qrchick-manager-quick{display:flex;gap:6px;flex-wrap:wrap;padding:0 12px 10px}.qrchick-manager-quick{}#qrchick-manager-quick button{border:1px solid rgba(148,163,184,.14);background:rgba(15,23,42,.38);color:#bfdbfe;border-radius:10px;padding:7px 9px;font-size:10px;cursor:pointer}#qrchick-manager-messages{flex:1;overflow:auto;padding:8px 12px}.qrchick-msg{display:flex;gap:8px;margin-bottom:10px}.qrchick-msg.user{justify-content:flex-end}.qrchick-msg-avatar{width:26px;height:26px;flex:0 0 26px}.qrchick-msg-avatar img{width:26px;height:26px;border-radius:50%;object-fit:cover}.qrchick-msg-body{max-width:88%}.qrchick-name{font-size:10px;color:#7dd3fc;margin:0 0 3px 3px}.qrchick-bubble{padding:9px 11px;border-radius:13px;background:rgba(30,41,59,.78);font-size:12px;line-height:1.48;color:#e2e8f0}.qrchick-msg.user .qrchick-bubble{background:rgba(37,99,235,.22);border:1px solid rgba(96,165,250,.16)}.qrchick-action-list{display:grid;gap:7px;margin-top:7px}.qrchick-action{padding:9px;border:1px solid rgba(148,163,184,.16);border-radius:12px;background:rgba(15,23,42,.4)}.qrchick-action-title{font-weight:700;font-size:11px}.qrchick-action-reason{font-size:10px;color:#94a3b8;margin-top:3px}.qrchick-action-preview{font-size:10px;color:#bfdbfe;margin-top:6px}.qrchick-action-buttons{display:flex;gap:6px;margin-top:7px}.qrchick-action-buttons button{flex:1;border:1px solid rgba(52,211,153,.25);background:rgba(16,185,129,.1);color:#bbf7d0;border-radius:8px;padding:7px;font-size:10px;cursor:pointer}.qrchick-action-buttons button.secondary{border-color:rgba(148,163,184,.18);background:rgba(15,23,42,.45);color:#cbd5e1}.qrchick-action-status{font-size:10px;margin-top:5px;color:#94a3b8}.qrchick-manager-input{display:flex;gap:8px;padding:10px 12px;border-top:1px solid rgba(148,163,184,.12)}.qrchick-manager-input textarea{flex:1;resize:none;border:1px solid rgba(148,163,184,.18);background:rgba(15,23,42,.5);color:#fff;border-radius:11px;padding:8px 10px;font-size:12px;outline:none}.qrchick-manager-input button{width:42px;border:1px solid rgba(62,196,255,.3);background:rgba(24,96,155,.28);color:#e0f2fe;border-radius:11px;cursor:pointer}.qrchick-manager-status{padding:6px 12px 8px;color:#64748b;font-size:9px;text-align:right}.qrchick-code{white-space:pre-wrap;font-size:10px;line-height:1.45;margin:6px 0;background:#020617;padding:8px;border-radius:8px;overflow:auto}';document.head.appendChild(style);
    document.getElementById('qrchick-manager-fab').onclick=function(){state.open=!state.open;render();};
    document.getElementById('qrchick-manager-close').onclick=function(){state.open=false;render();};
    document.getElementById('qrchick-manager-send').onclick=function(){send();};
    document.getElementById('qrchick-manager-input').addEventListener('keydown',function(e){if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();}});
    render();
    return true;
  }
  function boot(){if(install())return;setTimeout(boot,250);}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
  window.addEventListener('qr-manager-subscription-ready',function(){state.entitlementsReady=true;render();});
  window.addEventListener('qr-manager-ai-entitlements-updated',function(){state.entitlementsReady=true;render();});
  window.addEventListener('qr-manager-vue-ready',function(){state.entitlementsReady=true;render();});
})();