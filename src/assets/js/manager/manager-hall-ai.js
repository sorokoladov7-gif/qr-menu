/* QR Menu — canonical manager hall compatibility bridge. */
(function(){
  'use strict';
  ...
  /* Single owner of the manager venue RPC compatibility layer. */
  function patchDb(){
    ...
    try{
      ...
      function signature(items){
        return normProducts(items).map(function(x){return x.name+'|'+x.price+'|'+x.category;}).sort().join('\n');
      }
      async function resolveTemplateId(products){
        var r=await window.db.from('menu_templates').select('id,products').eq('is_active',true);
        if(r.error)throw r.error;
        var incoming=signature(products),list=r.data||[];
        var found=list.find(function(t){return signature(t.products)===incoming;});
        if(found)return found.id;
        throw new Error('Не удалось определить шаблон каталога. Обновите шаблоны меню.');
      }

      window.db.rpc=function(fn,args,options){
        if(fn!=='create_venue_for_manager'||!args||!Array.isArray(args.p_products))return originalRpc(fn,args,options);
        return resolveTemplateId(args.p_products).then(function(templateId){
          return originalRpc('create_venue_from_template',{
            p_template_id:templateId,
            p_name:args.p_name,
            p_slug:args.p_slug,
            p_plan:args.p_plan||'start',
            p_subscription_end:args.p_subscription_end
          },options);
        });
      };
  ...
  function patchVue(Vue){
    ...
    Vue.__QR_MANAGER_PATCH_V22__=true;
      ...
      if(options&&typeof options==='object'){
        ...
        if(typeof options.methods.selectVenueTemplate!=='function'){
          options.methods.selectVenueTemplate=function(id){
            ...
            if(typeof this.decorateVenueTemplateCards==='function'){
        ...
        options.methods.loadManagerSubscription=async function(){
          try{
            var s=await loadManagerSubscription(this);
            this.managerSubscription=s;
            if(s&&s.current_period_end)this.subscriptionEnd=s.current_period_end;
            return s;
          }catch(e){
            console.error('[QR Manager] manager subscription:',e);
        ...
        options.computed.managerPlan=function(){
          ...
          var p=s?plans.find(function(x){return x.id===s.plan_id;}):null;
          return p||this.currentPlan||null;
        };
        options.computed.venueLimit=function(){
          var p=this.managerPlan,n=p&&Number(p.max_venues);
        ...
        /* Do not replace canCreateVenue here. manager-app.js is the canonical owner. */
      }
