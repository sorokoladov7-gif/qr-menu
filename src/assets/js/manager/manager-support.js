...
(function(){
  ...
  function openSupport(){
    var v=vm();if(!v){setTimeout(openSupport,100);return;}
    v.tab='support';
    var p=makePanel();p.style.display='block';
    ensureNav();loadMessages();
    if(state.poll)clearInterval(state.poll);state.poll=setInterval(function(){var x=vm();if(x&&x.tab==='support')loadMessages();},10000);
  }
  function sync(){var v=vm();if(!v)return;ensureNav();var p=state.panel;if(v.tab==='support'){if(!p)p=makePanel();p.style.display='block';}else if(p)p.style.display='none';}

  function boot(){
    function tick(){ensureNav();sync();}
    tick();setInterval(tick,500);
    var root=document.getElementById('app');if(root)new MutationObserver(function(){setTimeout(tick,0);}).observe(root,{subtree:true,childList:true});
  }
