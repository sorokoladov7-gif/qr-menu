...
(function(){
  ...
  function ensureNav(){
    ...
    if(!b){
      ...
      b.type='button';b.setAttribute('data-qr-support-nav','1');b.textContent='🛟 Поддержка';
      tabs.appendChild(b);
    }
  ...
  function makePanel(){
    ...
    p.innerHTML='<div style="width:100%;min-height:calc(100dvh - 48px);box-sizing:border-box;display:flex;flex-direction:column;max-width:none"><div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:14px;flex:0 0 auto"><div><h3 style="margin:0">Поддержка</h3><div style="font-size:12px;color:#94a3b8;margin-top:4px">Напишите администратору — сообщение появится у него в кабинете.</div></div><div style="display:flex;align-items:center;gap:10px"><span data-support-status style="font-size:12px;color:#94a3b8"></span><button type="button" data-support-close class="btn" style="white-space:nowrap">✕ Закрыть</button></div></div><div data-support-messages style="flex:1;min-height:0;overflow:auto;padding:14px;border-radius:12px;background:rgba(2,6,23,.35)"></div><div style="display:flex;gap:8px;margin-top:12px;flex:0 0 auto"><textarea data-support-text placeholder="Опишите вопрос..." style="flex:1;min-height:80px;resize:vertical;box-sizing:border-box"></textarea><button type="button" data-support-send class="btn">Отправить</button></div></div>';
  ...
  async function ensureThread(){
    ...
    state.threadId=r.data;return state.threadId;
  }
  async function loadMessages(){
    try{var id=await ensureThread();var r=await db.from('manager_support_messages').select('id,thread_id,sender_role,sender_id,message,created_at,read_at').eq('thread_id',id).order('created_at',{ascending:true});if(r.error)throw r.error;state.messages=r.data||[];renderMessages();var s=state.panel&&state.panel.querySelector('[data-support-status]');if(s)s.textContent='';}
  ...
  function renderMessages(){
    ...
    box.innerHTML=state.messages.length?state.messages.map(function(m){var mine=m.sender_role==='manager';return '<div style="display:flex;justify-content:'+(mine?'flex-end':'flex-start')+';margin:7px 0"><div style="max-width:78%;padding:9px 11px;border-radius:12px;background:'+(mine?'rgba(99,102,241,.18)':'rgba(148,163,184,.10)')+'"><div style="font-size:11px;color:#94a3b8;margin-bottom:4px">'+(mine?'Вы':'Администратор')+' · '+esc(fmt(m.created_at))+'</div><div style="white-space:pre-wrap;word-break:break-word">'+esc(m.message)+'</div></div></div>';}).join(''):'<div style="color:#94a3b8;text-align:center;padding:45px 10px">Сообщений пока нет. Напишите администратору.</div>';
    box.scrollTop=box.scrollHeight;
  }
  async function send(){
    var p=makePanel(),ta=p.querySelector('textarea'),btn=p.querySelector('[data-support-send]'),text=String(ta.value||'').trim();if(!text)return;btn.disabled=true;
    try{var id=await ensureThread();var r=await db.rpc('manager_support_send',{p_thread_id:id,p_message:text});if(r.error)throw r.error;ta.value='';await loadMessages();}
    catch(e){p.querySelector('[data-support-status]').textContent='Ошибка: '+(e.message||e);}finally{btn.disabled=false;ta.focus();}
  }
  function showSupport(){var p=makePanel();p.style.display='block';loadMessages();if(state.poll)clearInterval(state.poll);state.poll=setInterval(function(){var v=vm();if(v&&v.tab==='support')loadMessages();},10000);}
  ...
  function boot(){function tick(){sync();}tick();setInterval(tick,500);var root=document.getElementById('app');if(root){state.navObserver=new MutationObserver(function(){setTimeout(tick,0);});state.navObserver.observe(root,{subtree:true,childList:true});}}
