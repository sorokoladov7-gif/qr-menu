...
async function renderMenuPages(urls) {
  ...
  try {
    ...
    const args = [...(Array.isArray(chromium.args) ? chromium.args : []), '--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage'];
    ...
    for (const url of targets) {
      ...
      try {
        ...
        await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/147 Safari/537.36 QR-Menu-Importer/2.0');
        const jsonResponses = [];
        page.on('response', async response => {
          try {
            ...
            if (body && body.length <= 1024 * 1024) { jsonResponses.push({ url: requestUrl, body }); diagnostics.network_json_responses++; }
          } catch (_) {}
        });

        await page.goto(url, { waitUntil: 'networkidle2', timeout: NAV_TIMEOUT }).catch(async error => {
          if (String(error?.message || '').includes('timeout')) await page.goto(url, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT }); else throw error;
        });
        await new Promise(resolve => setTimeout(resolve, SETTLE_MS));
        await page.evaluate(async () => {
          const step = Math.max(450, Math.floor(window.innerHeight * 0.8));
          for (let i = 0; i < 18; i++) { window.scrollBy(0, step); await new Promise(r => setTimeout(r, 140)); }
          window.scrollTo(0, 0);
        });
        ...
        const data = await page.evaluate(() => {
          ...
          const candidateName = (root, price) => {
          ...
          const priceElements = [...document.querySelectorAll('body *')].filter(el => {
            const t = text(el); return t && t.length <= 900 && (priceRe.test(t) || el.hasAttribute('data-price') || el.hasAttribute('itemprop') && el.getAttribute('itemprop') === 'price');
          });
          for (const el of priceElements) {
            ...
            for (let i = 0; i < 5 && root.parentElement; i++) {
              ...
              if (rt.length >= 20 && rt.length <= 1800) {
                const name = candidateName(root, price);
                if (name) {
          ...
          for (let i = 0; i < bodyLines.length; i++) {
            const m = bodyLines[i].match(priceRe);
            if (!m) continue;
            const price = m[0];
            for (let d = 1; d <= 5; d++) {
              for (const idx of [i - d, i + d]) {
                ...
                if (/[A-Za-zА-Яа-яЁё]/.test(name)) { add(name, price, '', null, 'rendered-text-near-price'); break; }
              }
              if (result.length) break;
            }
          }

          return { title: document.title, html: document.documentElement.outerHTML, text: text(document.body), products: result, priceNodes: priceElements.length };
        });
        ...
        for (const item of jsonResponses) { try { walkJson(JSON.parse(item.body)); } catch (_) {} }
        ...
        diagnostics.pages.push({ url, title: clean(data.title, 180), price_nodes: data.priceNodes, dom_products: data.products.length, network_products: jsonProducts.length, json_responses: jsonResponses.length, rendered_text_chars: data.text.length });
      } catch (error) {
