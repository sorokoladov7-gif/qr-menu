...
const {H,POS,DEL,MAP,fail,api,rpc,auth,entitlement,venue,num,str,product,venuePatch,resolveProduct}=require('./context');
...
async function run(c,f,a,e){
  ...
  if(['attach_ingredients','create_tech_card','save_recipe'].includes(type)){
    ...
    const ids=new Set(),norm=[];for(const r of rows){const iid=await require('./context').resolveIngredient(c,Object.assign({},r,{venue_id:vid})),q=num(r.quantity,NaN);if(!iid||!Number.isFinite(q)||q<=0||ids.has(iid))throw fail('RECIPE_ROW_INVALID',400);ids.add(iid);norm.push({ingredient_id:iid,quantity:q,note:r.note?str(r.note,500):null});}
