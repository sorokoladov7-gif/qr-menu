...
// These are core manager operations, not AI-only features. They must remain
// available to an authenticated manager even when the AI feature is disabled.
const CORE_OPERATIONAL_ACTIONS=new Set(['delete_staff','update_order']);
...
module.exports=async(req,res)=>{if(req.method!=='POST'){res.statusCode=405;return res.end(JSON.stringify({ok:false,error:'METHOD_NOT_ALLOWED'}))}try{
  const c=await auth(req),b=typeof req.body==='string'?JSON.parse(req.body||'{}'):(req.body||{}),f=str(b.feature||'assistant',40).toLowerCase(),action=b.action||{};
  const type=str(action?.type,80);
  // Do not gate core operational workflows behind the AI subscription.
  // Venue ownership and action-specific authorization are still enforced below.
  const e=CORE_OPERATIONAL_ACTIONS.has(type)?null:await entitlement(c,f);
  const r=await run(c,f,action,e);
  res.statusCode=200;res.setHeader('Content-Type','application/json; charset=utf-8');
  res.end(JSON.stringify({ok:true,feature:f,plan:e?.plan?.name||'core',result:r.result,type:r.type,status:r.status}));
}catch(e){res.statusCode=e.status||500;res.setHeader('Content-Type','application/json; charset=utf-8');res.end(JSON.stringify({ok:false,error:e.message||'MANAGER_AI_ACTION_FAILED'}));}};
