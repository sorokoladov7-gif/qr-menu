...
const TYPES=new Set(['recipe_auto_sync','attach_ingredients','create_tech_card','save_recipe']);
...
async function run(type,c,vid,p){
  ...
  if(type==='recipe_auto_sync'){
    ...
    return{type,status:'applied',result:await rpc('manager_recipe_auto_sync',{p_venue_id:vid,p_product_id:pid},c.token)};
  }
