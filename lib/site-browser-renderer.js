...
async function renderMenuPages(urls) {
  ...
  let browser;
  const diagnostics = { engine: 'puppeteer-core + @sparticuz/chromium', pages_attempted: targets.length, pages_rendered: 0, pages_failed: [], rendered_html_bytes: 0, network_json_responses: 0, price_nodes: 0, product_candidates: 0, products_found: 0, pages: [] };
    ...
    for (const url of targets) {
      ...
      page.on('response', async response => {
        try {
          ...
          if (body && body.length <= 1024 * 1024) {
            jsonResponses.push({ url: requestUrl, body: body.slice(0, 1024 * 1024) });
      ...
      try {
        ...
        await page.evaluate(async () => {
          ...
          for (let i = 0; i < 12; i++) { window.scrollBy(0, step); await new Promise(r => setTimeout(r, 100)); }
          window.scrollTo(0, 0);
        });
        ...
        const data = await page.evaluate(() => {
          ...
          for (const el of elements) {
            ...
            const explicit = el.getAttribute('data-price') || el.getAttribute('content');
            if (!m && !explicit) continue;
            const rawPrice = explicit || (m && m[0]);
            const priceMatch = String(rawPrice || '').match(/\d[\d\s.,]*/);
            if (!priceMatch) continue;
            priceNodes++;
            const candidates = [...el.querySelectorAll('h1,h2,h3,h4,h5,h6,[itemprop="name"],[class*="name" i],[class*="title" i]')].map(text).filter(x => x && !noiseRe.test(x) && !weightRe.test(x));
          ...
          if (!result.length) {
            for (const el of priceElements) {
              const t = text(el), m = t.match(priceRe);
              if (!m) continue;
              priceNodes++;
              let parent = el;
        ...
        for (const item of data.products) {
          ...
          seen.add(key); delete item.key; item.source_url = url; products.push(item);
        }
