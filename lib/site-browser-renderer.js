...
async function renderMenuPages(urls) {
  ...
  try {
    ...
    browser = await puppeteer.launch({ args, defaultViewport: { width: 1365, height: 900, deviceScaleFactor: 1 }, executablePath, headless: 'shell', ignoreHTTPSErrors: true });
      ...
      try {
        ...
        page.on('response', async response => {
          try {
            ...
            if (body && body.length <= 512 * 1024) { jsonResponses.push({ url: requestUrl, body: body.slice(0, 512 * 1024) }); diagnostics.network_json_responses++; }
          } catch (_) {}
        });
        ...
        await page.evaluate(async () => {
          ...
          for (let i = 0; i < 10; i++) { window.scrollBy(0, step); await new Promise(resolve => setTimeout(resolve, 100)); }
          window.scrollTo(0, 0);
        });
        ...
        const data = await page.evaluate(() => {
          ...
          const result = [];
          let priceNodes = 0;
          const add = (name, price, description, image) => {
            ...
            result.push({ key, name, price: priceValue, description: String(description || '').slice(0, 600), image_url: image || null, source: 'rendered-dom' });
          };
          ...
          for (const el of elements) {
            ...
            add(names[0] || parts[0], priceMatch[0], names[1] || parts.slice(1, 3).join(' '), img?.currentSrc || img?.src || img?.dataset?.src || null);
          }
        ...
        for (const item of data.products) { const key = item.name.toLowerCase(); if (seen.has(key)) continue; seen.add(key); delete item.key; item.source_url = url; products.push(item); }
        ...
        diagnostics.pages_failed.push({ url, code: error?.name === 'TimeoutError' ? 'BROWSER_TIMEOUT' : 'BROWSER_PAGE_FAILED', message: String(error?.message || error).slice(0, 300) });
    ...
    return { ok: false, code: 'BROWSER_ENGINE_FAILED', message: String(error?.message || error), pages: [], products: [], diagnostics: { ...diagnostics, engine_error_name: error?.name || 'Error', engine_error_message: String(error?.message || error) } };
