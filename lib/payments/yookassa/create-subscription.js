const { json, supabase, getSupabaseUser, bearer, origin, yookassaCreatePlatformPayment, yookassaGetPlatformPayment } = require('../../_lib/yookassa');
  ...
  try {
    ...
    if (subscription.payment_id) {
      try {
        ...
        if (String(metadata.qr_menu_subscription_id || '') === subscription.id && String(metadata.qr_menu_plan_id || '') === targetPlan.id) {
          if (['pending', 'waiting_for_capture', 'succeeded'].includes(String(payment.status || '').toLowerCase())) {
