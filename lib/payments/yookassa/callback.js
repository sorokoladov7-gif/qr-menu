...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (providerError || !code) {
      await supabase(`payment_oauth_states?id=eq.${encodeURIComponent(oauthState.id)}&consumed_at=is.null`, {
    ...
    const consumed = await supabase(`payment_oauth_states?id=eq.${encodeURIComponent(oauthState.id)}&consumed_at=is.null&select=id`, {
      method: 'PATCH', headers: { Prefer: 'return=representation' },
      body: JSON.stringify({ consumed_at: new Date().toISOString() })
    });
    if (!Array.isArray(consumed) || !consumed.length) {
      return json(res, 400, { ok: false, error: 'oauth_state_consumed' });
    }
