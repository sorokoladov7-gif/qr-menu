...
async function yookassaToken(code,codeVerifier,redirectUri){const clientId=env('YOOKASSA_CLIENT_ID');const clientSecret=env('YOOKASSA_CLIENT_SECRET');const basic=Buffer.from(`${clientId}:${clientSecret}`).toString('base64');const params={grant_type:'authorization_code',code};if(codeVerifier)params.code_verifier=codeVerifier;if(redirectUri)params.redirect_uri=redirectUri;const body=new URLSearchParams(params);const response=await fetch('https://yookassa.ru/oauth/v2/token',{method:'POST',headers:{Authorization:`Basic ${basic}`,'Content-Type':'application/x-www-form-urlencoded'},body});const data=await response.json();if(!response.ok||!data.access_token){const error=new Error(data.error_description||data.error||'ЮKassa не вернула OAuth-токен');error.status=response.status;throw error;}return data;}
...
module.exports={env,json,redirect,randomToken,sha256,pkceChallenge,supabase,getSupabaseUser,bearer,assertManagerVenue,callbackUrl,origin,yookassaToken,yookassaMe,yookassaCreatePayment,yookassaGetPayment,yookassaCreatePlatformPayment,yookassaGetPlatformPayment,encryptSecret,decryptSecret};
