const { json, randomToken, supabase, yookassaCreatePayment, decryptSecret, origin } = require('./core');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (order.payment_id && order.payment_provider === 'yookassa') {
      try {
        const { yookassaGetPayment } = require('./core');
    ...
    if (Array.isArray(transactionRows) && transactionRows.length) await supabase(`payment_transactions?id=eq.${encodeURIComponent(transactionRows[0].id)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify(transactionPayload) });
    else await supabase('payment_transactions', { method: 'POST', headers: { Prefer: 'return=minimal' }, body: JSON.stringify(transactionPayload) });
