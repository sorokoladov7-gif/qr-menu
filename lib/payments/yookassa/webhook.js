...
async function getVenuePayment(paymentId, venueId) {
  let accounts = await supabase(`payment_accounts?venue_id=eq.${encodeURIComponent(venueId)}&provider=yookassa&account_scope=eq.venue&status=eq.active&select=credentials_ref&limit=1`);
  ...
  if (!account) {
    ...
    if (managerId) {
      accounts = await supabase(`payment_accounts?manager_id=eq.${encodeURIComponent(managerId)}&account_scope=eq.platform&venue_id=is.null&provider=yookassa&status=eq.active&select=credentials_ref&limit=1`);
