...
function ledgerStatus(status) {
  return ['succeeded', 'canceled', 'waiting_for_capture', 'pending', 'failed', 'refunded'].includes(status) ? status : 'failed';
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (subscription) {
      const payment = await yookassaGetPlatformPayment(paymentId);
      const status = paymentStatus(payment.status);
      if (status === 'paid') {
        ...
        if (!alreadyApplied) {
          ...
          const patch = {
            ...
            status: 'active',
            current_period_end: new Date(start + days * 86400000).toISOString()
          };
          if (targetPlanId) patch.plan_id = targetPlanId;
          const claimed = await supabase(`subscriptions?id=eq.${encodeURIComponent(subscription.id)}&payment_status=neq.paid&paid_at=is.null`, {
            method: 'PATCH', headers: { Prefer: 'return=representation' }, body: JSON.stringify(patch)
          });
          if (!Array.isArray(claimed) || !claimed.length) {
            return json(res, 200, { ok: true, type: 'subscription', payment_id: paymentId, status: payment.status, already_applied: true });
          }
        }
      } else {
        await supabase(`subscriptions?id=eq.${encodeURIComponent(subscription.id)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ payment_status: status }) });
      }
