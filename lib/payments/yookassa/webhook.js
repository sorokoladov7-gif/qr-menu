const { json, supabase, decryptSecret, yookassaGetPlatformPayment } = require('./core');
...
function paymentStatus(status) { if (status === 'succeeded') return 'paid'; if (status === 'canceled') return 'cancelled'; if (status === 'failed') return 'failed'; if (status === 'refunded') return 'refunded'; return 'pending'; }
function ledgerStatus(status) { return ['succeeded', 'canceled', 'waiting_for_capture', 'pending', 'failed', 'refunded'].includes(status) ? status : 'failed'; }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (order) {
      ...
      const status = paymentStatus(payment.status), patch = { payment_status: status };
    ...
    const subscriptions = await supabase(`subscriptions?payment_id=eq.${encodeURIComponent(paymentId)}&select=id,venue_id,manager_id,plan_id,payment_id,payment_status,current_period_end,paid_at&limit=1`);
    const subscription = Array.isArray(subscriptions) ? subscriptions[0] : null;
    if (subscription) {
      const payment = await yookassaGetPlatformPayment(paymentId), status = paymentStatus(payment.status);
      if (status === 'paid') {
        const alreadyApplied = subscription.payment_status === 'paid' && subscription.paid_at;
        if (!alreadyApplied) {
          ...
          const patch = { payment_status: 'paid', paid_at: new Date().toISOString(), status: 'active', current_period_end: new Date(start + days * 86400000).toISOString() };
          if (targetPlanId) patch.plan_id = targetPlanId;
          const claimed = await supabase(`subscriptions?id=eq.${encodeURIComponent(subscription.id)}&payment_status=neq.paid&paid_at=is.null`, { method: 'PATCH', headers: { Prefer: 'return=representation' }, body: JSON.stringify(patch) });
          if (!Array.isArray(claimed) || !claimed.length) return json(res, 200, { ok: true, type: 'subscription', payment_id: paymentId, status: payment.status, already_applied: true });
        }
      } else await supabase(`subscriptions?id=eq.${encodeURIComponent(subscription.id)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ payment_status: status }) });
