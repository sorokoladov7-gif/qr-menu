...
function rawPrices(html) {
  ...
  while ((m = re.exec(safe)) && out.length < 2000) {
    ...
    if (price > 0 && price < 1000000) out.push({ price, index: m.index, sample: clean(safe.slice(Math.max(0, m.index - 220), m.index + m[0].length + 220), 600) });
  }
  return out;
}
  ...
  for (const block of blocks) {
    try {
      ...
      const walk = value => {
        ...
        if ((type.includes('product') || type.includes('menuitem') || type.includes('menu') || value.offers) && value.name && price > 0 && price < 1000000) {
...
function textNearPrices(html) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (!price) continue;
    const candidates = [];
    for (let d = 1; d <= 12; d++) { candidates.push(list[i - d], list[i + d]); }
...
function addProduct(products, keys, item, pageUrl, category = 'main') {
  ...
  keys.add(key);
  products.push({ name, description: clean(item.description, 600), price, category: clean(item.category || category || 'main', 120), image_url: item.image_url || null, is_available: true, applies_to: 'all', source_url: pageUrl, extraction_source: item.source || 'analyzer-v3' });
  return true;
}

function menuLinkScore(url, anchorText = '', title = '') {
  let score = 0;
  if (MENU_URL_RE.test(String(url || ''))) score += 8;
  if (MENU_TEXT_RE.test(anchorText)) score += 8;
  if (MENU_TEXT_RE.test(title)) score += 5;
  return score;
}

function pageCategory(html) {
  const hit = lines(html).find(x => CATEGORY_RE.test(x));
  return hit ? clean(hit, 120) : 'main';
}
...
async function analyzeSite(inputUrl) {
  ...
  const diagnostics = {
    version: '33.0', pages_attempted: 0, pages_checked: 0, pages_failed: [], links_found: 0, html_bytes: 0,
    ...
    confidence: 0, confidence_reasons: [], js_render: { required: false, pages: [] }, price_samples: [], analysis_steps: []
  };
  const headers = { 'User-Agent': 'Mozilla/5.0 QR-Menu-Site-Analyzer/33.0', Accept: 'text/html,application/xhtml+xml,application/json,text/plain,*/*;q=0.8', 'Accept-Language': 'ru-RU,ru;q=0.9,en;q=0.8' };
  ...
  for (const page of pages) {
    const evidence = { url: page.url, raw_price_hits: 0, structural_cards: 0, jsonld_hits: 0, embedded_hits: 0, nearby_hits: 0, product_candidates: 0, accepted_products: 0 };
    const category = pageCategory(page.html);

    const raw = rawPrices(page.html);
    evidence.raw_price_hits = raw.length;
    ...
    evidence.structural_cards = structural.length;
    diagnostics.structural_cards += structural.length;
    for (const item of structural) {
      evidence.product_candidates++;
      if (addProduct(products, keys, item, page.url, category)) evidence.accepted_products++;
    }

    const jsonld = extractJsonLd(page.html, page.url);
    evidence.jsonld_hits = jsonld.length;
    ...
    evidence.embedded_hits = embedded.length;
    ...
    evidence.nearby_hits = nearby.length;
    ...
    pageEvidence.set(page.url, evidence);
  }

  // Core rule: a page becomes a menu page because real priced product positions
  // were extracted from it. URL/category names are only discovery hints.
  for (const evidence of pageEvidence.values()) {
    if (evidence.product_candidates <= 0) continue;
    const strong = evidence.structural_cards > 0 || evidence.jsonld_hits > 0 || evidence.embedded_hits > 0 || evidence.nearby_hits >= 2;
    if (!strong) continue;
    diagnostics.menu_pages_by_products.push(evidence.url);
    diagnostics.menu_page_evidence.push({
      url: evidence.url,
      product_candidates: evidence.product_candidates,
      accepted_products: evidence.accepted_products,
      structural_cards: evidence.structural_cards,
      raw_price_hits: evidence.raw_price_hits,
      jsonld_hits: evidence.jsonld_hits,
      embedded_hits: evidence.embedded_hits,
      nearby_hits: evidence.nearby_hits
    });
  }

  diagnostics.menu_pages = [...new Set([...diagnostics.menu_pages, ...diagnostics.menu_pages_by_products])];
  diagnostics.menu_pages_by_products = [...new Set(diagnostics.menu_pages_by_products)];
  diagnostics.menu_page_evidence.sort((a, b) => (b.accepted_products - a.accepted_products) || (b.product_candidates - a.product_candidates));
  diagnostics.menu_pages.sort((a, b) => {
    const ae = diagnostics.menu_page_evidence.find(x => x.url === a);
    const be = diagnostics.menu_page_evidence.find(x => x.url === b);
    return (be?.accepted_products || 0) - (ae?.accepted_products || 0) || menuLinkScore(b) - menuLinkScore(a);
  });

  diagnostics.products_found = products.length;
  const signals = [];
  if (diagnostics.menu_pages_by_products.length) signals.push({ points: 25, reason: `страницы подтверждены реальными позициями меню: ${diagnostics.menu_pages_by_products.length}` });
  else if (diagnostics.menu_pages.length) signals.push({ points: 10, reason: `найдены страницы-кандидаты меню: ${diagnostics.menu_pages.length}` });
  if (diagnostics.structural_cards >= 3) signals.push({ points: 30, reason: `найдены структурные карточки товаров: ${diagnostics.structural_cards}` });
  ...
  diagnostics.analysis_steps.push(`Извлечение: ${products.length} товаров; подтверждено страниц меню: ${diagnostics.menu_pages_by_products.length}; confidence ${diagnostics.confidence}%`);
    ...
    for (const block of blocks) {
      try {
        ...
        const walk = x => {
          ...
          if (!business.name && /restaurant|cafe|bar|foodestablishment|localbusiness/i.test(String(x['@type'] || ''))) business = x;
  ...
  return {
    ...
    venue: {
      ...
      logo_url: business.logo ? absolute(typeof business.logo === 'string' ? business.logo : business.logo.url, start.href) : null,
      opening_hours: business.openingHours || null
    },
    products: products.slice(0, 500),
    meta: {
      menu_found: validated,
      products_found: products.length,
      pages_checked: pages.length,
      pages_discovered: Math.max(0, seen.size - 1),
      best_menu_page: diagnostics.menu_pages[0] || null,
      validation: validated ? 'validated' : 'not_validated',
      diagnostics,
      error
    }
  };
