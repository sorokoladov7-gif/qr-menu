...
async function analyzeSite(inputUrl) {
  ...
  while (queue.length && pages.length < MAX_PAGES) {
    ...
    const links = [...page.html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/giu)];
    diagnostics.links_found += links.length;
    for (const match of links.slice(0, MAX_LINKS)) {
      const href = absolute(match[1], page.url);
      if (!href || !sameHost(href, host) || ASSET_RE.test(href)) continue;
      const normalized = href.replace(/#.*$/, '');
      if (seen.has(normalized)) continue;
      seen.add(normalized);
      const anchorText = clean(visibleText(match[2]), 120);
      const score = menuLinkScore(normalized, anchorText, title);
