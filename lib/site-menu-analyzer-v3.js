...
function addProduct(products, keys, raw, pageUrl, category = 'main') {
  ...
  keys.add(key);
  products.push({ name, description: clean(raw?.description, 600), price, category: clean(raw?.category || category || 'main', 120), image_url: raw?.image_url || null, is_available: true, applies_to: 'all', source_url: pageUrl, extraction_source: raw?.source || 'analyzer-v4' });
  return true;
}
    ...
    try {
      ...
      const walk = value => {
        ...
        if ((/(product|menuitem)/i.test(type) || value.offers) && value.name && price > 0 && price < 1000000) {
          ...
          out.push({ name: value.name, description: value.description || '', price, image_url: image, category: value.category, source: 'json-ld' });
        }
...
function extractEmbedded(html) {
  const out = [];
  const re = /(?:"|')(?:name|title|productName|dishName)(?:"|')\s*:\s*(?:"|')([^"']{2,220})(?:"|')[\s\S]{0,900}?(?:"|')(?:price|cost|amount)(?:"|')\s*:\s*(?:"|')?([\d\s.,]{1,12})/giu;
  let m;
  while ((m = re.exec(String(html || ''))) && out.length < 700) {
    const price = Number(m[2].replace(/\s/g, '').replace(',', '.'));
    if (price > 0 && price < 1000000) out.push({ name: m[1], price, source: 'embedded-json' });
  }
  return out;
}
...
function extractCards(html, pageUrl) {
  ...
  while ((m = re.exec(String(html || ''))) && out.length < 500) {
    ...
    const image = block.match(/<(?:img|source)[^>]+(?:src|data-src|data-lazy-src|data-original)=["']([^"']+)/iu)?.[1] || null;
    out.push({ name: candidates[0], description: candidates.slice(1, 3).join(' '), price, image_url: absolute(image, pageUrl), source: 'structural-card' });
  }
  return out;
}
...
async function readWithJina(url) {
  ...
  try {
    ...
    const response = await fetch(endpoint, { signal: controller.signal, headers: { 'user-agent': 'QR-Menu-Site-Analyzer/42.0' } });
...
async function analyzeSite(inputUrl) {
  ...
  const diagnostics = {
    version: '42.0', pages_attempted: 0, pages_checked: 0, pages_failed: [], links_found: 0, html_bytes: 0,
  ...
  if (!products.length && pdfTargets.length && Date.now() - started < ANALYZER_BUDGET) {
    ...
    pdfResults.forEach(text => { if (text) { diagnostics.pdf_fallback.successful++; for (const item of pdfProducts(text)) addProduct(products, keys, item, pdfTargets[Math.min(diagnostics.pdf_fallback.successful - 1, pdfTargets.length - 1)].url, 'main'); } });
