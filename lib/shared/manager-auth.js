...
async function getProfile(userId, accessToken) {
  ...
  const key=serviceKey||SUPABASE_PUBLIC_KEY;
  const headers={apikey:key,Accept:'application/json'};
  headers.Authorization=`Bearer ${serviceKey||accessToken}`;
  const response=await fetch(`${SUPABASE_URL}/rest/v1/profiles?id=eq.${encodeURIComponent(userId)}&select=id,email,display_name,role&limit=1`,{headers});
...
async function getManagerUser(accessToken){
  ...
  if(cronSecret&&accessToken.length===cronSecret.length&&crypto.timingSafeEqual(Buffer.from(accessToken),Buffer.from(cronSecret))) return {id:'cron',email:null,user_metadata:{},role:'admin',internal:true};
  try{
  ...
  const profile=await getProfile(claims.sub,accessToken);if(!profile||profile.id!==claims.sub||!['manager','admin'].includes(profile.role))throw Object.assign(new Error('manager_or_admin_access_required'),{status:403});
...
async function getManagerUserOrCron(req) {
  const secret=String(process.env.CRON_SECRET||'');
  const internal=String((req&&req.headers&&req.headers['x-qr-cron-secret'])||'');
  const a=Buffer.from(internal),b=Buffer.from(secret);
  if(secret&&internal&&a.length===b.length&&crypto.timingSafeEqual(a,b)) return {id:'cron',email:null,user_metadata:{},role:'admin',internal:true};
  return getManagerUser(bearer(req));
}
module.exports={bearer,getManagerUser,getManagerUserOrCron};
