  ...
  try{
    ...
    for(const row of rows||[]){
      try{
        ...
        results.push({venue_id:row.venue_id,provider:row.provider,ok:!result.error_count,received:result.received,created:result.created,updated:result.updated,unchanged:result.unchanged,errors:result.error_count});
      }catch(e){
        results.push({venue_id:row.venue_id,provider:row.provider,ok:false,error:e.message||String(e)});
      }
    }
