...
function relevance(path,message,mode){const q=(String(message||'')+' '+String(mode||'')).toLowerCase();let score=0;const p=path.toLowerCase();if(/admin|ai|gemini/.test(p))score+=30;if(/api|supabase|rpc|rls|auth/.test(p))score+=20;if(/import|menu|pdf|site/.test(q)&&/import|menu|pdf|site/.test(p))score+=35;if(/security|безопас|rls|auth/.test(q)&&/security|auth|rls|supabase|admin/.test(p))score+=25;if(/recipe|ingredient|tech/.test(q)&&/recipe|ingredient|tech/.test(p))score+=25;if(/manager/.test(q)&&/manager/.test(p))score+=20;if(/\.html$/.test(p))score+=5;if(/package\.json|vercel\.json|config/.test(p))score+=10;return score;}

async function repoSnapshot(message='',mode='full'){
  const tree=await gh('/git/trees/'+encodeURIComponent(GITHUB_BRANCH)+'?recursive=1');
  const files=(tree.tree||[]).filter(x=>x.type==='blob'&&!shouldSkip(x.path)&&/\.(js|mjs|cjs|ts|tsx|html|css|json|sql|md)$/i.test(x.path)).sort((a,b)=>relevance(b.path,message,mode)-relevance(a.path,message,mode)).slice(0,MAX_FILES);
  ...
  for(const f of files){
    try{
      ...
      const text=await r.text();const part='===== FILE: '+f.path+' =====\n'+text.slice(0,MAX_FILE_CHARS);if(total+part.length>MAX_CONTEXT_CHARS)continue;chunks.push(part);total+=part.length;
...
async function supabaseManagementQuery(query,readOnly){
  if(!SUPABASE_MANAGEMENT_API_TOKEN)throw err('SUPABASE_MANAGEMENT_API_TOKEN_NOT_CONFIGURED',503);const endpoint=readOnly?'/database/query/read-only':'/database/query';
...
async function supabaseSnapshot(){
  if(SUPABASE_MANAGEMENT_API_TOKEN){try{
  ...
  try{const r=await fetch(SUPABASE_URL+'/rest/v1/',{headers:{apikey:SUPABASE_SERVICE_ROLE_KEY,authorization:'Bearer '+SUPABASE_SERVICE_ROLE_KEY,accept:'application/openapi+json'}});const d=await r.json().catch(()=>null);if(!r.ok)return {available:false,error:'SUPABASE_HTTP_'+r.status};return {available:true,source:'postgrest_openapi',tables:Object.keys(d?.definitions||{}).slice(0,500)};}catch(e){return {available:false,error:clean(e?.message||'SUPABASE_UNAVAILABLE',300)};}
}
...
async function callGemini(promptText){
  ...
  try{
    const payload={contents:[{role:'user',parts:[{text:clip(promptText,MAX_PROMPT_CHARS)]}]}],generationConfig:{responseMimeType:'application/json',maxOutputTokens:24000,thinkingConfig:{thinkingLevel:'low'}}};
    const r=await fetch('https://generativelanguage.googleapis.com/v1beta/models/'+GEMINI_MODEL+':generateContent',{method:'POST',signal:controller.signal,headers:{'x-goog-api-key':GEMINI_API_KEY,'content-type':'application/json'},body:JSON.stringify(payload)});const d=await r.json().catch(()=>null);
    if(!r.ok){const msg=d?.error?.message||'GEMINI_HTTP_'+r.status;if(r.status===429||/quota|rate.?limit|exceed/i.test(msg))throw err('GEMINI_QUOTA_EXCEEDED: '+clean(msg,900),429);throw err(msg,r.status);}
...
function buildPrompt(s,message,mode,history){
  const freeChat=mode==='chat';
  return [
    'Ты — основной инженерный AI-помощник администратора платформы QR Menu.',
    'Твоя роль шире аудита: свободно общайся с администратором, отвечай на вопросы, объясняй архитектуру, анализируй ошибки, помогай разрабатывать, отлаживать, улучшать UI/UX, API, Supabase, RLS/RPC, импорт меню, Vercel и GitHub.',
    'Работай как опытный senior/full-stack инженер: сначала пойми задачу, затем предложи конкретное решение и при необходимости подготовь точные изменения существующих файлов.',
    'Не ограничивай ответ форматом аудита. Если администратор просто задаёт вопрос — ответь прямо и по существу.',
    'Не выдумывай файлы, строки, таблицы, колонки, функции, ошибки или результаты тестов. Отделяй подтверждённые факты от гипотез.',
    'Для GitHub разрешены только изменения существующих файлов через отдельное подтверждение администратора. Новые файлы не создавай.',
    ...
    'Если проблема требует изменения кода, укажи файл, причину, конкретный участок и безопасный вариант исправления.',
    'Если задача связана с импортом меню, учитывай PDF на много страниц, фотографии, сайт, OCR/извлечение текста, категории, блюда, ингредиенты и нормализацию.',
    'Если данных недостаточно, скажи что именно нужно проверить, вместо выдумывания результата.',
    'Режим: '+clean(mode||'chat',30),
    'Контекст диалога: '+safeJson(Array.isArray(history)?history.slice(-16):[],18000),
    'Запрос администратора: '+String(message||'').slice(0,14000),
    'Верни JSON с полями summary, answer, severity, root_cause, findings, actions, files, confidence, safe_to_change, proposed_changes, database_changes. Поле answer должно содержать полноценный свободный ответ администратору.',
    ...
    'GITHUB SNAPSHOT: '+safeJson(s.git,270000),
    'VERCEL DEPLOYMENTS: '+safeJson(s.vercel,18000),
    'VERCEL RUNTIME: '+safeJson(s.runtime,22000),
    'SUPABASE: '+safeJson(s.supabase,50000),
...
async function handler(req,res){
  ...
  try{
    ...
    if(action==='db_probe'){const database=await supabaseSnapshot();return res.status(200).json({ok:true,admin:admin.email,database,capabilities:{supabase_database_read:!!SUPABASE_MANAGEMENT_API_TOKEN,supabase_database_write:!!SUPABASE_MANAGEMENT_API_TOKEN}});}
    if(action!=='audit')return res.status(400).json({error:'UNKNOWN_ACTION'});
    const mode=body.mode||'chat',message=body.message||'';
    const git=await repoSnapshot(message,mode),vercel=await vercelSnapshot(),runtime=await vercelRuntimeSnapshot(vercel),supabase=await supabaseSnapshot();
    const result=await callGemini(buildPrompt({git,vercel,runtime,supabase,context:git.context},message,mode,body.history||[]));
  ...
  }catch(e){const status=Number(e?.status)||500;console.error('[admin-ai-audit]',e);return res.status(status).json({error:clean(e?.message||'AI_AUDIT_FAILED',1200)});}
}
