...
module.exports = async function(req, res) {
  ...
  try {
    ...
    const renderTargets = [...new Set([...menuPages, ...jsPages])].slice(0, 2);
    ...
    if (shouldRender && renderTargets.length) {
      ...
      if (browserResult.ok) {
        ...
        if (result.products.length >= 5) { meta.menu_found = true; meta.validation = 'validated-browser-rendered'; meta.error = null; }
      }
      if (result.products.length < 5) {
        if (browserResult.code === 'BROWSER_ENGINE_FAILED' || browserResult.code === 'BROWSER_DEPENDENCY_FAILED') {
          meta.error = { code: 'MENU_BROWSER_RENDER_FAILED', message: 'Меню требует браузерного JavaScript-анализа, но серверный Chromium недоступен.', details: { browser_code: browserResult.code, browser: browserResult.diagnostics || {}, original: normalizeError(meta.error) } };
        } else if (diagnostics.js_render?.required) {
          meta.error = { code: 'MENU_JS_RENDERED_NOT_EXTRACTED', message: 'Страница была обработана браузером, но товарные позиции не удалось извлечь.', details: { browser: browserResult.diagnostics || {}, original: normalizeError(meta.error) } };
        }
      }
