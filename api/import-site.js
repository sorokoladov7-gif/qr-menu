module.exports = async function handler(req, res) {
  ...
  const headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/131 Safari/537.36 QR-Menu-Importer/8.0','Accept':'text/html,application/xhtml+xml,text/plain;q=0.9,*/*;q=0.8','Accept-Language':'ru-RU,ru;q=0.9,en-US;q=0.7,en;q=0.5'};
  ...
  async function getReader(url){const r=await fetch('https://r.jina.ai/'+url,{headers:{'User-Agent':'QR-Menu-Importer/8.0','Accept':'text/plain'},signal:AbortSignal.timeout(30000)});if(!r.ok)throw Error('reader_'+r.status);return {text:(await r.text()).slice(0,12*1024*1024),url,mode:'text'};}
  ...
  const candidates=[];
  const menuWords=/(menu|меню|catalog|каталог|food|dish|блюд|еда|price|цены|ассортимент|bar|бар|пицц|суш|ролл|напит|dessert|десерт)/i;
  if(pages[0].mode==='html'){
    const re=/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;let m;
    while((m=re.exec(pages[0].html))&&candidates.length<250){const u=abs(m[1],pages[0].url),txt=clean(m[2].replace(/<[^>]+>/g,' '),250);if(u&&same(u)&&menuWords.test(u+' '+txt))candidates.push(u.split('#')[0])}
  }
  const common=['/menu','/Menu','/menu/','/menyu','/menyu/','/catalog','/catalog/','/food','/food/','/dishes','/dishes/','/restaurant/menu','/restaurant/menu/'];
  for(const p of common){const u=abs(p,target.href);if(u&&!candidates.includes(u))candidates.push(u)}
  ...
  for(const pg of sourcePages){
    ...
    for(let i=0;i<ls.length;i++){
      ...
      // Search both before and after price. Lemon-style pages commonly emit price/weight/name.
      const window=[];for(let j=Math.max(0,i-5);j<Math.min(ls.length,i+10);j++)window.push({x:ls[j],d:j-i});
      const after=window.filter(o=>o.d>0&&!weight.test(o.x)&&!noise.test(o.x)&&!price(o.x)&&o.x.length>=2&&o.x.length<=180);
      const before=window.filter(o=>o.d<0&&!weight.test(o.x)&&!noise.test(o.x)&&!price(o.x)&&o.x.length>=2&&o.x.length<=180);
      let name=after.find(o=>o.d<=4)?.x||before.slice(-1)[0]?.x;
      if(name){const desc=after.filter(o=>o.x!==name).slice(0,4).map(o=>o.x).join(' ');add(name,p,desc,category,null)}
    }
  }
  // Generic fallback: price followed by a short title, independent of a fixed category list.
  if(products.length<3){for(const pg of sourcePages){const ls=lines(pg);for(let i=0;i<ls.length;i++){const p=price(ls[i]);if(!p)continue;for(let j=i+1;j<Math.min(i+8,ls.length);j++){const x=ls[j];if(price(x)||weight.test(x)||noise.test(x))continue;if(x.length>=3&&x.length<=180){add(x,p,'','main',null);break}}}}}
  // Structured data is supplementary.
