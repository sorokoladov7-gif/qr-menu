module.exports = async function handler(req, res) {
  ...
  const headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/131 Safari/537.36 QR-Menu-Importer/7.0','Accept':'text/html,application/xhtml+xml,text/plain;q=0.9,*/*;q=0.8','Accept-Language':'ru-RU,ru;q=0.9,en-US;q=0.7,en;q=0.5'};
  ...
  async function getReader(url){const r=await fetch('https://r.jina.ai/'+url,{headers:{'User-Agent':'QR-Menu-Importer/7.0','Accept':'text/plain'},signal:AbortSignal.timeout(30000)});if(!r.ok)throw Error('reader_'+r.status);return {text:(await r.text()).slice(0,12*1024*1024),url,mode:'text'};}
  ...
  const candidates=[];
  if(pages[0].mode==='html'){const re=/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;let m;while((m=re.exec(pages[0].html))&&candidates.length<250){const u=abs(m[1],pages[0].url),txt=clean(m[2].replace(/<[^>]+>/g,' '),200);if(u&&same(u)&&!/[.]pdf(?:$|[?#])/i.test(u)&&/(menu|меню|catalog|каталог|food|dish|блюд|еда|price|цены|ассортимент|bar|бар|пицц|суш|ролл)/i.test(u+' '+txt))candidates.push(u.split('#')[0])}}
  for(const p of ['/menu','/Menu','/menyu','/menu/','/catalog','/catalog/','/food','/dishes']){const u=abs(p,target.href);if(u&&!candidates.includes(u))candidates.push(u)}
  for(const u of [...new Set(candidates)].slice(0,18)){try{pages.push(await getHtml(u))}catch(e){try{pages.push(await getReader(u))}catch(x){}}}
  ...
  const categoriesRe=/^(новинки|закуски|салаты|супы|горячие блюда(?: из (мяса|рыбы))?|паста|бургеры|пицца|специи и соусы|соусы|суши|классические роллы|авторские роллы|жаренные роллы|запеченные роллы|роллы|десерты|детское меню|детское|барная карта|напитки|завтраки|основные блюда|горячее|рыба|мясо)$/i;
  const noise=/^(меню|главная|о нас|интерьер|доставка|акции|контакты|резерв стола|забронировать|заказать|добавить в корзину|калории|белки|жиры|углеводы|наш адрес|адрес|телефон|разработка сайта|вход|регистрация)$/i;
  const weight=/^\d+(?:[.,]\d+)?\s*(?:г|гр|кг|мл|л|шт)\.?$/i;
  const products=[],seen=new Set(),categories=new Set();
  function add(name,p,desc,cat,img){name=clean(name,180);p=Number(p||0);if(!name||!p||p<1||p>1000000||noise.test(name)||weight.test(name))return;const key=name.toLowerCase();if(seen.has(key))return;seen.add(key);products.push({name,description:clean(desc,1200)||null,price:p,category:clean(cat,120)||'main',image_url:img||null,is_available:true,applies_to:'all'})}
  const all=[];for(const pg of pages)for(const l of lines(pg))all.push({l,pg});
  let category='main';
  for(let i=0;i<all.length;i++){
    const l=all[i].l;
    if(categoriesRe.test(l)){category=l;categories.add(l);continue}
    const p=price(l);if(!p)continue;
    let name=null,desc=[];
    for(let j=i+1;j<Math.min(i+14,all.length);j++){const x=all[j].l;if(categoriesRe.test(x))break;if(price(x))break;if(weight.test(x)||noise.test(x)||/^\d+$/.test(x))continue;if(!name&&x.length>=2&&x.length<=180){name=x;continue}if(name&&desc.join(' ').length<1200)desc.push(x)}
    if(name)add(name,p,desc.join(' '),category,null);
  }
  // Fallback for pages whose visible menu is supplied as one large text block.
  if(products.length<3){for(const pg of pages){const ls=lines(pg);for(let i=0;i<ls.length;i++){const p=price(ls[i]);if(!p)continue;const after=ls.slice(i+1,i+9).filter(x=>!weight.test(x)&&!noise.test(x)&&!price(x)&&x.length>1);const name=after.find(x=>x.length<=180);if(name)add(name,p,after.filter(x=>x!==name).join(' '),'main',null)}}}
  // Structured data is supplementary, never the sole menu source.
