...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    let accounts = await supabase(`payment_accounts?venue_id=eq.${encodeURIComponent(order.venue_id)}&provider=eq.yookassa&account_scope=eq.venue&status=eq.active&select=id,credentials_ref,merchant_id,metadata&limit=1`);
    ...
    return json(res, 200, { ok: true, payment_id: payment.id, status: payment.status, confirmation_url: confirmationUrl });
  } catch (e) {
