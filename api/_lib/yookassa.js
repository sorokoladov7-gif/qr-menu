...
function supabaseConfig(){
  return {
    ...
    authKey: PUBLIC_SUPABASE_ANON_KEY
  };
}
async function supabase(path,options={}){
  ...
  const response=await fetch(`${cfg.url}/rest/v1/${path}`,Object.assign({},options,{headers}));
...
async function getSupabaseUser(accessToken){
  const cfg=supabaseConfig();
  if(!cfg.url||!accessToken)return null;
  const response=await fetch(`${cfg.url}/auth/v1/user`,{headers:{apikey:cfg.authKey,Authorization:`Bearer ${accessToken}`}});
  if(!response.ok){let details=null;try{details=await response.json();}catch(_){}const error=new Error('supabase_user_auth_failed');error.status=response.status;error.data=details;throw error;}
...
async function yookassaCreatePayment(accessToken,payload,idempotenceKey,shopId){const headers={Authorization:`Bearer ${accessToken}`,'Idempotence-Key':idempotenceKey||randomToken(24),'Content-Type':'application/json'};if(shopId)headers['On-Behalf-Of']=String(shopId);const response=await fetch('https://api.yookassa.ru/v3/payments',{method:'POST',headers,body:JSON.stringify(payload)});const data=await response.json();if(!response.ok||!data||!data.id){const error=new Error(data&&(data.description||data.message||data.code)||`ЮKassa HTTP ${response.status}`);error.status=response.status;error.data=data;throw error;}return data;}
async function yookassaGetPayment(accessToken,paymentId,shopId){const headers={Authorization:`Bearer ${accessToken}`};if(shopId)headers['On-Behalf-Of']=String(shopId);const response=await fetch(`https://api.yookassa.ru/v3/payments/${encodeURIComponent(paymentId)}`,{headers});const data=await response.json();if(!response.ok||!data||data.id!==paymentId){const error=new Error(data&&(data.description||data.message)||`ЮKassa HTTP ${response.status}`);error.status=response.status;error.data=data;throw error;}return data;}
async function yookassaListWebhooks(accessToken,shopId){const response=await fetch(`https://api.yookassa.ru/v3/webhooks/${encodeURIComponent(shopId)}`,{headers:{Authorization:`Bearer ${accessToken}`,'On-Behalf-Of':String(shopId)}});const data=await response.json().catch(function(){return {};});if(!response.ok){const error=new Error(data&&(data.description||data.message)||`ЮKassa webhooks HTTP ${response.status}`);error.status=response.status;error.data=data;throw error;}return Array.isArray(data.items)?data.items:[];}
async function yookassaRegisterWebhook(accessToken,shopId,event,url){const response=await fetch(`https://api.yookassa.ru/v3/webhooks/${encodeURIComponent(shopId)}/new`,{method:'POST',headers:{Authorization:`Bearer ${accessToken}`,'On-Behalf-Of':String(shopId),'Content-Type':'application/json'},body:JSON.stringify({event,url})});const data=await response.json().catch(function(){return {};});if(!response.ok||!data||!data.id){const error=new Error(data&&(data.description||data.message||data.code)||`ЮKassa webhook register HTTP ${response.status}`);error.status=response.status;error.data=data;throw error;}return data;}
async function yookassaDeleteWebhook(accessToken,shopId,webhookId){const response=await fetch(`https://api.yookassa.ru/v3/webhooks/${encodeURIComponent(shopId)}/${encodeURIComponent(webhookId)}/destroy`,{method:'POST',headers:{Authorization:`Bearer ${accessToken}`,'On-Behalf-Of':String(shopId)}});return true;}
...
module.exports={env,json,redirect,randomToken,sha256,supabase,getSupabaseUser,bearer,assertManagerVenue,callbackUrl,origin,yookassaToken,yookassaMe,yookassaCreatePayment,yookassaGetPayment,yookassaListWebhooks,yookassaRegisterWebhook,yookassaDeleteWebhook,yookassaCreatePlatformPayment,yookassaGetPlatformPayment,encryptSecret,decryptSecret};
