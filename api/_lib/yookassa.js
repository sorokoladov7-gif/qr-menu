...
// OAuth PKCE (S256) + configurable YooKassa OAuth endpoints (defaults match YooKassa's current paths).
function pkceChallenge(verifier){return base64url(crypto.createHash('sha256').update(String(verifier)).digest());}
function authorizeUrl(){return process.env.YOOKASSA_AUTHORIZE_URL || 'https://yookassa.ru/oauth/v2/authorize';}
function tokenUrl(){return process.env.YOOKASSA_TOKEN_URL || 'https://yookassa.ru/oauth/v2/token';}
...
async function yookassaToken(code,opts={}){const clientId=env('YOOKASSA_CLIENT_ID');const clientSecret=env('YOOKASSA_CLIENT_SECRET');const basic=Buffer.from(`${clientId}:${clientSecret}`).toString('base64');const params={grant_type:'authorization_code',code};if(opts.redirectUri)params.redirect_uri=opts.redirectUri;if(opts.codeVerifier)params.code_verifier=opts.codeVerifier;const body=new URLSearchParams(params);const response=await fetch(tokenUrl(),{method:'POST',headers:{Authorization:`Basic ${basic}`,'Content-Type':'application/x-www-form-urlencoded'},body});const data=await response.json().catch(function(){return {};});if(!response.ok||!data.access_token){const error=new Error(data.error_description||data.error||'ЮKassa не вернула OAuth-токен');error.status=response.status;error.data=data;throw error;}return data;}
...
module.exports={env,json,redirect,randomToken,sha256,pkceChallenge,authorizeUrl,supabase,getSupabaseUser,bearer,assertManagerVenue,callbackUrl,origin,yookassaToken,yookassaMe,yookassaCreatePayment,yookassaGetPayment,yookassaCreatePlatformPayment,yookassaGetPlatformPayment,encryptSecret,decryptSecret};
