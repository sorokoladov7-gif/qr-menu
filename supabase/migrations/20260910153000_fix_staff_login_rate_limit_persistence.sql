...
-- A failed login cannot raise after updating the limiter because PostgreSQL
-- rolls back the function's writes when the exception escapes. Return the
-- existing {error: ...} contract instead so the failure counter commits.
