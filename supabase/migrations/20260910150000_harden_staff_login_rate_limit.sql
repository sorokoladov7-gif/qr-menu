...
declare
  ...
  v_locked_until timestamptz;
begin
  -- Auto-clean expired sessions, preserving the existing login behavior.
  delete from public.staff_sessions
   where expires_at < now() - interval '7 days';

  if p_type not in ('cook','courier','waiter') then
    raise exception 'invalid_staff_type';
  end if;

  if nullif(trim(p_slug),'') is null or nullif(trim(p_pin),'') is null then
    raise exception 'slug_or_pin_required';
  end if;
  ...
  -- Supabase/PostgREST supplies the client address through X-Forwarded-For.
  -- Keep the key textual so a missing header cannot make inet parsing fail.
  v_ip_key := coalesce(
    nullif(
      split_part(
  ...
  -- Serialize attempts for the same IP/venue/staff type to avoid concurrent
  -- requests bypassing the threshold.
  perform pg_advisory_xact_lock(
    hashtextextended(v_ip_key || '|' || v_venue_id::text || '|' || p_type, 0)
  );
  ...
  select window_started, failed_attempts, locked_until
   ...
   where ip_key = v_ip_key
     ...
     and staff_type = p_type
   for update;

  if v_locked_until is not null and v_locked_until > now() then
    raise exception 'staff_login_rate_limited';
  end if;
  ...
  if v_staff_id is null then
    ...
    insert into private.staff_login_rate_limits (
      ip_key, venue_id, staff_type, window_started, failed_attempts,
      locked_until, last_attempt_at
    ) values (
      v_ip_key, v_venue_id, p_type, v_window_started, v_failed_attempts,
      case when v_failed_attempts >= 5 then now() + interval '15 minutes' else null end,
      now()
    )
    on conflict (ip_key, venue_id, staff_type)
    do update set
  ...
  -- Successful authentication clears the failure window for this key.
  ...
  insert into public.staff_sessions (staff_type, staff_id, venue_id, token, expires_at)
