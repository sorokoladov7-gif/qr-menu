...
declare v_role text;
begin
  select case when p.role='admin' then 'admin' else 'manager' end into v_role
  from public.profiles p where p.id=auth.uid() and p.role in ('admin','manager');
  ...
  insert into public.manager_support_typing(thread_id,user_id,sender_role,is_typing,updated_at)
  values(p_thread_id,auth.uid(),v_role,coalesce(p_is_typing,false),now())
  on conflict(thread_id,user_id) do update set sender_role=excluded.sender_role,is_typing=excluded.is_typing,updated_at=now();
end; $$;
...
returns table(sender_role text,is_typing boolean,updated_at timestamptz)
language sql security definer set search_path=public as $$
  select x.sender_role,x.is_typing,x.updated_at from public.manager_support_typing x
  where x.thread_id=p_thread_id and x.updated_at>now()-interval '5 seconds' and x.is_typing=true;
$$;
